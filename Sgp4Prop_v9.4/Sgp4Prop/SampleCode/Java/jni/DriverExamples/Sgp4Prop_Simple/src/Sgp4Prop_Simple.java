/*
      ...
      1. Load and initialize all the required libraries. This step has already been handled
	     automatically by implementing Java's static block in each library's JNI wrapper class
...
public class Sgp4Prop_Simple
{
   public static void main(String[] args) throws IOException
   {
	   ...
	   satKey = JniTle.TleAddSatFrLines("1 90021U RELEAS14 00051.47568104 +.00000184 +00000+0 +00000-4 0 0814",
								        "2 90021   0.0222 182.4923 0000720  45.6036 131.8822  1.00271328 1199");

	   // other ways to load TLEs into memory to work with
	   //JniTle.TleLoadFile(fileName);  // load TLEs from a text file
	   //JniTle.TleAddSatFrFieldsGP();  // load a TLE by passing its data fields
	   ...
	   for (double ds50UTC = startTime; ds50UTC < endTime; ds50UTC += 0.5)
	   {
		  ...
		  JniSgp4Prop.Sgp4PropDs50UTC(satKey, ds50UTC, mse, pos, vel, llh); // see Sgp4Prop dll document
		  // other available propagation methods
		  //JniSgp4Prop.Sgp4PropDs50UtcLLH(satKey, ds50UTC, llh);
		  //JniSgp4Prop.Sgp4PropDs50UtcPos(satKey, ds50UTC, pos);
	   }
	   ...
	   for (double mse = 0; mse < (30 * 1440); mse += 1440)
	   {
		  ...
		  JniSgp4Prop.Sgp4PropMse(satKey, mse, ds50UTC, pos, vel, llh); // see Sgp4Prop dll document

		   System.out.println(String.format("ds50UTC: %10.8f", ds50UTC[0]));
		   for( int ii=0; ii<3; ii++ ) {
			   System.out.println(String.format("pos: %10.15f vel: %10.15f llh: %10.15f", pos[ii], vel[ii], llh[ii]));
		   }
	   }
	   ...
	   //JniTle.TleRemoveAllSats();   // remove all loaded TLEs from memory
	   //JniSgp4Prop.Sgp4RemoveAllSats();  // remove all initialized TLEs from memory
   }
}
