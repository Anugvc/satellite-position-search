...
public class SrvLamod
{
   ...
   public static void PrintSenSatHeader(BufferedWriter outFile, long senKey, long satKey, int stepModeReq, byte viewType, byte obsType, int optVisFlg) throws IOException
   {
      ...
      JniLamod.LamodGet1pField(JniLamod.XF_1P_STEPMODE, tempVal);
      ...
      JniSensor.SensorGetLimField(senKey, JniSensor.XF_SENLIM_UNIT, tempVal);
      int units = Utility.BytesToInt(tempVal);
      String cun = (units != 0) ? "nm" : "km";
      outFile.write(String.format("LOOK ANGLES FOR SATELLITE %5d%14sSENSOR %3d\n", JniElOps.SatNumOf(satKey), " ", JniSensor.SenNumOf(senKey)));
      tempVal = new byte[512];
      JniSatState.SatStateGetSatDataField(satKey, JniSatState.XF_SAT_ELSETNUM, tempVal);
      int elsetNum = Utility.BytesToInt(tempVal);
      tempVal = new byte[512];
      JniSatState.SatStateGetSatDataField(satKey, JniSatState.XF_SAT_EPOCH, tempVal);
   ...
   public static void PrintLook(BufferedWriter outFile,  long senSatKey, byte viewType, byte obsType, int optVisFlg, byte currStep, double [] lookArr) throws IOException
   {
      ...
      // What it is is that Jni always needs reference,
      ...
      double ds50UTC;
      double[] rasc = new double [1];
      double[] decl = new double [1];
      int[] irahr = new int [1];
      int[] iramn = new int [1];
      int[] idecdg = new int [1];
      int[] idecmn = new int [1];
      double[] xrasc = new double [1];
      double[] xdecsc = new double [1];
      String currTimeStr;
      double [] angArr = new double[5];
      String decSignStr;
      double [] azElRates = new double[2];

      ds50UTC = lookArr[JniLamod.XA_LOOK_DS50UTC];
      elevation   = lookArr[JniLamod.XA_LOOK_ELEV];
      ...
      azimuth   = lookArr[JniLamod.XA_LOOK_AZIM];
      range  = lookArr[JniLamod.XA_LOOK_RNG];
      rangeRate  = lookArr[JniLamod.XA_LOOK_RNGRT];
      ...
      JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_RELVEL, relvel);
      JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_ANGARR, angArr);
      saa = angArr[JniLamod.XA_ANGLE_SOLARASPECT];

      if (obsType == '5' || obsType == '9' || obsType == 'O')
      {
         JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_RUVEC, xlPos);

         JniLamod.LamodCompRaDec(xlPos, rasc, decl, irahr, iramn, xrasc, idecdg, idecmn, xdecsc);
         if (idecdg[0] == 0 && decl[0] < 0.0)
         {
            decSignStr = "-0";
         }
         else
         {
            decSignStr = String.format("%03d", idecdg[0]);
         }
      }

      JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_CVIS, cvisd);
      cvis = (byte) cvisd[0];
      ...
      if (obsType == '8' || obsType == 'M')
      {
         JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_SENEFG, dvel);
      ...
      {
         JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_SENEFG, dvel);
         outFile.write(String.format("%18s  %7.3f  %7.3f %10.3f %8.3f  %2d %2d %6.3f  %s %2d %5.2f   %8.3f  %c%c %2s%10.3f%10.3f%10.3f\n",
            currTimeStr, elevation, azimuth, range, relvel[0], irahr[0], iramn[0], xrasc[0], idecdg[0], idecmn[0], xdecsc[0],
            saa, ' ', cvis, cobp, dvel[0], dvel[1], dvel[2]));
      }
      else if (obsType == '5')
      {
         outFile.write(String.format("%s  %7.3f  %7.3f %10.3f %8.3f  %2d %2d %6.3f  %s %2d %5.2f %c %8.3f    %c %2s\n",
         currTimeStr, elevation, azimuth, range, rangeRate, irahr[0], iramn[0], xrasc[0], idecdg[0], idecmn[0],
         xdecsc[0], currStep, saa, cvis, cobp));
      }
      // DHN 15Aug11 - Add azimuth and elevation rates for obsType 4
      // <JCL> Rewrote this to be a single else if followed by else instead of nesting
      else if(obsType == '4')
      {
         JniLamod.LamodGetViewDataField(senSatKey, JniLamod.XF_VIEW_AZELRATES, azElRates);
   ...
   public static void PrintObs(BufferedWriter outFile, long senSatKey, int punchObs) throws IOException
   {
      ...
      JniLamod.LamodGenObs(senSatKey, punchObs, 'U', obsLine1, obsLine2, numLines);
   ...
   public static void PrintNosticData(BufferedWriter outFile, int senNum, int satNum, char viewType, int nosticFlg, double[] lookArr, int lookCode) throws IOException
   {
      ...
      if (nosticFlg >= 2)
      {
         ...
         outFile.write(String.format("   >>> Sensor: %3d - Satellite: %6d - Time: %20s - Mse: %14.6f\n", senNum,
                  satNum, SrvTimeFunc.UTCToDtg20Str(lookArr[JniLamod.XA_LV_DS50UTC]), lookArr[JniLamod.XA_LV_MSE]));
   ...
   public static void PrintPassByPassNostic(BufferedWriter outFile, char viewType, double[] lookArr) throws IOException
   {
     String msgStrFormat = "    --- The sensor/satellite FAILED %s test ---\n";
      ...
      if ((int)lookArr[JniLamod.XA_LV_MINRANGE] == 0)
         outFile.write(String.format(msgStrFormat, "minimum range"));

      if ((int)lookArr[JniLamod.XA_LV_MAXRANGE] == 0)
   ...
   */
   ...
   {
      outFile.write(String.format("    Sensor Position: X   =%10.3f km;  Y   =%10.3f km;  Z   =%10.3f km \n", lookArr[JniLamod.XA_LV_SENPOSX], lookArr[JniLamod.XA_LV_SENPOSY], lookArr[JniLamod.XA_LV_SENPOSZ]));
      outFile.write(String.format("           Velocity: XD  =%10.5f k/s; YD  =%10.5f k/s; ZD  =%10.5f k/s\n", lookArr[JniLamod.XA_LV_SENVELX], lookArr[JniLamod.XA_LV_SENVELY], lookArr[JniLamod.XA_LV_SENVELZ]));
      outFile.write(String.format("           Geod    : nLat=%10.5f deg; eLon=%10.5f deg; Hght=%10.3f km \n", lookArr[JniLamod.XA_LV_SENLAT], lookArr[JniLamod.XA_LV_SENLON], lookArr[JniLamod.XA_LV_SENHEIGHT]));
      outFile.write(String.format("    Sat Position   : X   =%10.3f km;  Y   =%10.3f km;  Z   =%10.3f km \n", lookArr[JniLamod.XA_LV_SATPOSX], lookArr[JniLamod.XA_LV_SATPOSY], lookArr[JniLamod.XA_LV_SATPOSZ]));
      outFile.write(String.format("        Velocity   : XD  =%10.5f k/s; YD  =%10.5f k/s; ZD  =%10.5f k/s\n", lookArr[JniLamod.XA_LV_SATVELX], lookArr[JniLamod.XA_LV_SATVELY], lookArr[JniLamod.XA_LV_SATVELZ]));
      outFile.write(String.format("        Geod       : nLat=%10.5f deg; eLon=%10.5f deg; Hght=%10.3f km \n", lookArr[JniLamod.XA_LV_SATLAT], lookArr[JniLamod.XA_LV_SATLON], lookArr[JniLamod.XA_LV_SATHEIGHT]));


      // For orbiting sensors
      if (viewType == '9')
      {
         outFile.write(String.format("    Solar Unit Vec : X   =%14.8f      Y   =%14.8f      Z   =%14.8f    \n", lookArr[JniLamod.XA_LV_USUNX], lookArr[JniLamod.XA_LV_USUNY], lookArr[JniLamod.XA_LV_USUNZ]));
         outFile.write(String.format("    Lunar Unit Vec : X   =%14.8f      Y   =%14.8f      Z   =%14.8f    \n", lookArr[JniLamod.XA_LV_UMOONX], lookArr[JniLamod.XA_LV_UMOONY], lookArr[JniLamod.XA_LV_UMOONZ]));
         outFile.write(String.format("    Angles (deg): Sun/sen/sat =%8.3f; Sun/earth/sat =%8.3f; Sun/earth/sen =%8.3f; Moon/sen/sat =%8.3f\n",
                 lookArr[JniLamod.XA_LV_SUSESA], lookArr[JniLamod.XA_LV_SUEASA], lookArr[JniLamod.XA_LV_SUEASE], lookArr[JniLamod.XA_LV_MOSESA]));

         if (lookArr[JniLamod.XA_LV_NBORE1] > 0)
         {
            outFile.write(String.format("    Off-boresight & clock angles (#1) =%8.3f & %8.3f deg\n",
                    lookArr[JniLamod.XA_LV_OBEL1], lookArr[JniLamod.XA_LV_OBAZ1]));

            if (lookArr[JniLamod.XA_LV_NBORE2] > 0 && lookArr[JniLamod.XA_LV_NBORE2] < 7)
               outFile.write(String.format("    Off-boresight & clock angles (#2) =%8.3f & %8.3f deg\n",
                       lookArr[JniLamod.XA_LV_OBEL2], lookArr[JniLamod.XA_LV_OBAZ2]));
         }
      }
      outFile.write("\n");
   }
