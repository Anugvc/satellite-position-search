...
/** This class defines the JNI component of the wrapper for: Vcm */
public class JniVcm
{
   ...
   */
   public static native void LoadVcmDll();
   ...
   */
   public static native void VcmAddSatFrLinesML(String vcmString, long[] satKey);
   ...
   * @param satKey             The satKey of the newly added VCM on success, a negative value on error (out-Long)
   */
   public static native void VcmAddSatFrFieldsML(String xs_vcm, double[] xa_vcm, long[] satKey);
   ...
   * @return ! 0 if the VCM is successfully retrieved, non-0 if there is an error
   */
   public static native int VcmGetAllFields(long satKey, int[] satNum, byte[] satName, byte[] epochDtg, int[] revNum, double[] posECI, double[] velECI, byte[] geoName, int[] geoZonals, int[] geoTesserals, byte[] dragModel, byte[] lunarSolar, byte[] radPress, byte[] earthTides, byte[] intrackThrust, double[] bTerm, double[] agom, double[] ogParm, double[] cmOffset, int[] f10, int[] f10Avg, double[] apAvg, double[] tconRec, int[] nTerms, byte[] leapYrDtg, byte[] integMode, byte[] partials, byte[] stepMode, byte[] fixStep, byte[] stepSelection, double[] initStepSize, double[] errCtrl, double[] rms);
   ...
   */
   public static native void VcmGetSatKeyML(int satNum, long[] satKey);
   ...
   * @param satKey             The satellite's unique key (out-Long)
   */
   public static native void VcmFieldsToSatKeyML(int satNum, String epochDtg, long[] satKey);
