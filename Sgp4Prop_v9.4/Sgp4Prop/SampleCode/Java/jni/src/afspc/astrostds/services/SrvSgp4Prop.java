...
public class SrvSgp4Prop
{
   public static int Xyz2Tle(int yr, double days, double[] posIn, double[] velIn, String tles[])
   {
      ...
      errCode = JniSgp4Prop.Sgp4PosVelToKep(yr, days, posIn, velIn, posOut, velOut, sgp4MeanKep);
      if(errCode != 0)
         return errCode;

      A     = sgp4MeanKep[JniAstroFunc.XA_KEP_A];
      ecc   = sgp4MeanKep[JniAstroFunc.XA_KEP_E];
      incl  = sgp4MeanKep[JniAstroFunc.XA_KEP_INCLI];
      node  = sgp4MeanKep[JniAstroFunc.XA_KEP_NODE];
      MA    = sgp4MeanKep[JniAstroFunc.XA_KEP_MA];
      omega = sgp4MeanKep[JniAstroFunc.XA_KEP_OMEGA];
      browerN = JniAstroFunc.AToN(A);
      
      // Convert Brouwer mean motion to Kozai mean motion
      kozaiN = JniAstroFunc.BrouwerToKozai(ecc, incl,  browerN);
      
      // Create GP TLE - ephemeris type 0
      JniTle.TleGPFieldsToLines(99999, 'U', "MySat", yr, days, 0.0, 0.0, 0.0, 0, 0, incl, node,
