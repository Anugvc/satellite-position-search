...
public class SrvElOps {
    ...
    public static long[] GetCurrLoadedSatKeys(int fileType)
    {
        ...
        if (fileType == FILETYPE_TLE)
            ...
            numSats = JniSpVec.SpVecGetCount();
        else if (fileType == FILETYPE_VCM)
            numSats = JniVcm.VcmGetCount();
        else if (fileType == FILETYPE_EXTEPH)
            numSats = JniExtEphem.ExtEphGetCount();

        if (numSats > 0)
        {
            tmpSatKeys = new long[numSats];
            JniTle.TleGetLoaded(2, tmpSatKeys);
        }
    ...
    public static long[] SatKeysFrFile(int fileType, String inputFile)
    {
        ...
        if (fileType == FILETYPE_TLE)
            errCode = JniTle.TleLoadFile(inputFile);
        else if (fileType == FILETYPE_SPVEC)
            errCode = JniSpVec.SpVecLoadFile(inputFile);
        else if (fileType == FILETYPE_VCM)
            errCode = JniVcm.VcmLoadFile(inputFile);
        else if (fileType == FILETYPE_EXTEPH)
            errCode = JniExtEphem.ExtEphLoadFile(inputFile);
    ...
    public static double ComputePerigeeHt(double mnMotion, double eccen)
    {
        ...
        perigeeHt = (JniAstroFunc.NToA(mnMotion) * (1 - eccen)) - JniEnvConst.EnvGetGeoConst(JniEnvConst.XF_GEOCON_KMPER);
