...
/** This class defines the JNI component of the wrapper for: Fov */
public class JniFov
{
   ...
   */
   public static native void LoadFovDll();
   ...
   */
   public static native void FovGetDataFrInputFiles(double[] xa_fovCtrl, int[] numOfWindows, double[] startStopTimes, int[] numOfSources, double[][] xa_fovSrc_Arr, int[] numOfTargets, double[][] xa_fovTgt_Arr, int[] numVicSats, int[] vicSatNums);


   /**
   * This function retrieves various FOV input data being entered from input flat files (various FOV cards)
   * @param xa_fovCtrl         FOV control parameters, see XA_FOVCTRL_? for array arrangement (out-Double[])
   * @param numOfWindows       Number of specified start/stop windows (out-Integer)
   * @param startStopTimes     Array of start/stop times in days since 1950 UTC (even idxs=start times, odd idxs=stop times) (out-Double[])
   * @param numOfSources       Number of sources (out-Integer)
   * @param xa_fovSrc_Arr      Array of source records, see XA_FOVSRC_? for array arrangement (out-Double[])
   * @param numOfTargets       Number of targets (out-Integer)
   * @param xa_fovTgt_Arr      Array of target records, see XA_FOVTGT_? for array arrangement (out-Double[])
   * @param numVicSats         Number of specified potential victims (out-Integer)
   * @param vicSatNums         Array of satellite numbers of the specified potential victims (out-Integer[])
   */
   public static native void FovGetDataFrInputFiles2Das1D(double[] xa_fovCtrl, int[] numOfWindows, double[] startStopTimes, int[] numOfSources, double[] xa_fovSrc_Arr, int[] numOfTargets, double[] xa_fovTgt_Arr, int[] numVicSats, int[] vicSatNums);
   ...
   */
   public static native int FovFindTargetPasses(double startTimeDs50UTC, double stopTimeDs50UTC, long srcSenKey, long tgtSatKey, double[][] tgtPassesArr, int[] numOfPasses);
   ...
   * @param srcSenKey          the source sensor key (in-Long)
   * @param tgtSatKey          the targeted satellite's unique key (in-Long)
   * @param tgtPassesArr       array of entry/exit times (in Ds50TAI) of each pass; the size of the array should match the specified value in XA_FOVRUN_MAXPASSES (out-Double[])
   * @param numOfPasses        number of passes that target satellite can be seen (passed all limit checks) by the source sensor (out-Integer)
   * @return 0 if successfully, non-0 if there is an error
   */
   public static native int FovFindTargetPasses2Das1D(double startTimeDs50UTC, double stopTimeDs50UTC, long srcSenKey, long tgtSatKey, double[] tgtPassesArr, int[] numOfPasses);
   ...
   * @return 0 if successfully, non-0 if there is an error
   */
   public static native int FovTargetElset(double[] xa_fovRun, double[] xa_fovSrc, long tgtSatKey, long vicSatKey, double[][] xa_emeDat, int[] numOfPasses);
   ...
   * <br>
   * See FovGetDataFrInputFiles for a description of the XA_FOVSRC parameter values.
   * @param xa_fovRun          fov run parameters, see XA_FOVRUN_? for array arrangement (in-Double[])
   * @param xa_fovSrc          source data, see XA_FOVSRC_? for array arrangement (in-Double[])
   * @param tgtSatKey          the targeted satellite's unique key (in-Long)
   * @param vicSatKey          the victim satellite's unique key (in-Long)
   * @param xa_emeDat          array of entry/minimum/exit (in Ds50TAI) times of each pass (out-Double[])
   * @param numOfPasses        number of passes found (out-Integer)
   * @return 0 if successfully, non-0 if there is an error
   */
   public static native int FovTargetElset2Das1D(double[] xa_fovRun, double[] xa_fovSrc, long tgtSatKey, long vicSatKey, double[] xa_emeDat, int[] numOfPasses);
   ...
   * @return 0 if successfully, non-0 if there is an error
   */
   public static native int FovTargetVec(double[] xa_fovRun, double[] xa_fovSrc, double[] xa_fovTgt, long vicSatKey, double[][] xa_emeDat, int[] numOfPasses);
   ...
   * @param xa_fovRun          fov run parameters, see XA_FOVRUN_? for array arrangement (in-Double[])
   * @param xa_fovSrc          source data, see XA_FOVSRC_? for array arrangement (in-Double[])
   * @param xa_fovTgt          target data, see XA_FOVTGT_? for array arrangement (in-Double[])
   * @param vicSatKey          the victim satellite's unique key (in-Long)
   * @param xa_emeDat          array of entry/minimum/exit (in Ds50TAI) times of each pass (out-Double[])
   * @param numOfPasses        number of passes found (out-Integer)
   * @return 0 if successfully, non-0 if there is an error
   */
   public static native int FovTargetVec2Das1D(double[] xa_fovRun, double[] xa_fovSrc, double[] xa_fovTgt, long vicSatKey, double[] xa_emeDat, int[] numOfPasses);
