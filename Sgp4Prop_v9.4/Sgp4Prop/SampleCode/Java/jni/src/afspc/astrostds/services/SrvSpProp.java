...
public class SrvSpProp
{
   ...
   public static void PrintSpAppCtrl(BufferedWriter outFile) throws IOException
   {
      ...
      int[] buffSize = new int [1];
      int[] runMode = new int [1];
      int[] savePartials = new int [1];
      int[] isSpectr = new int [1];
      double[] consider = new double [1];
      int[] decayAlt = new int [1];
      int[] outCoord = new int [1];
      byte [] truncGeoDir = new byte[60];
      JniSpProp.SpGetApCtrlAll(geoDir, buffSize, runMode, savePartials, isSpectr, consider, decayAlt, outCoord);
      ...
      outFile.write(String.format(" | Run Mode       : %-59s|\n", runMode[0] == 1 ? "Memory Priority" : "Performance Priority"));
      outFile.write(String.format(" | Specter Mode   : %-59s|\n", isSpectr[0] == 1 ? "Compatible" : "Not Compatible"));
