...
public class SrvBatchDC
{
	...
	public static String[] CreateNewGpTle(int[] xai_dcElts, double[] xar_dcElts)
	{
		int ephType;
		int[] year = new int [1];
		double[] daysOfYear = new double [1];
		double n;
		double agomOrNdotsqr;
		byte[] line1 = new byte[JniTle.XS_TLE_SIZE];
		byte[] line2 = new byte[JniTle.XS_TLE_SIZE];

		JniTimeFunc.UTCToYrDays(xar_dcElts[JniBatchDC.XAR_DCELTS_EPOCHDS50UTC], year, daysOfYear);

		n = xar_dcElts[JniBatchDC.XAR_DCELTS_EQNX_N]; // Brouwer mean motion


		if (xai_dcElts[JniBatchDC.XAI_DCELTS_ORBTYPE] == JniDllMain.ELTTYPE_TLE_XP) // XP
		{
			ephType = 4; // SGP4-XP DC
			agomOrNdotsqr = xar_dcElts[JniBatchDC.XAR_DCELTS_AGOM];
		}
		else
		{
			ephType = 0; // SGP4 DC
			agomOrNdotsqr = xar_dcElts[JniBatchDC.XAR_DCELTS_N2DOT];
			n = JniAstroFunc.BrouwerToKozai(xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_E], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_INCLI], n); // Kozai mean motion
		}

		// generate SP TLE from data fields
		JniTle.TleGPFieldsToLines(xai_dcElts[JniBatchDC.XAI_DCELTS_SATNUM], 'U', "GPDC", year[0], daysOfYear[0], xar_dcElts[JniBatchDC.XAR_DCELTS_NDOT],
				agomOrNdotsqr, xar_dcElts[JniBatchDC.XAR_DCELTS_BFIELD], ephType, xai_dcElts[JniBatchDC.XAI_DCELTS_ELSETNUM],
				xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_INCLI], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_NODE], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_E],
				xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_OMEGA], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_MA], n, xai_dcElts[JniBatchDC.XAI_DCELTS_REVNUM], line1, line2);
	...
	public static String[] CreateNewSpTle(int[] xai_dcElts, double[] xar_dcElts)
	{
		...
		// generate SP TLE from data fields
		JniTle.TleSPFieldsToLines(xai_dcElts[JniBatchDC.XAI_DCELTS_SATNUM], 'U', "SPDC", year[0], daysOfYear[0], xar_dcElts[JniBatchDC.XAR_DCELTS_BFIELD],
				xar_dcElts[JniBatchDC.XAR_DCELTS_OGPARM], xar_dcElts[JniBatchDC.XAR_DCELTS_AGOM], xai_dcElts[JniBatchDC.XAI_DCELTS_ELSETNUM],
				xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_INCLI], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_NODE], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_E],
				xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_OMEGA], xar_dcElts[JniBatchDC.XAR_DCELTS_KEP_MA], xar_dcElts[JniBatchDC.XAR_DCELTS_EQNX_N], xai_dcElts[JniBatchDC.XAI_DCELTS_REVNUM],
				line1, line2);
	...
	public static String[] CreateNewSpVec(int[] xai_dcElts, double[] xar_dcElts)
	{
		...
		byte[] line1 = new byte[JniSpVec.XS_SPVEC_SIZE];
		byte[] line2 = new byte[JniSpVec.XS_SPVEC_SIZE];

		pos[0] = xar_dcElts[JniBatchDC.XAR_DCELTS_POSX];
		pos[1] = xar_dcElts[JniBatchDC.XAR_DCELTS_POSY];
		pos[2] = xar_dcElts[JniBatchDC.XAR_DCELTS_POSZ];

		// SpVec takes velocity in m/s, need to convert vel from km/s to m/s
		velMPS[0] = xar_dcElts[JniBatchDC.XAR_DCELTS_VELX] * 1000;
		velMPS[1] = xar_dcElts[JniBatchDC.XAR_DCELTS_VELY] * 1000;
		velMPS[2] = xar_dcElts[JniBatchDC.XAR_DCELTS_VELZ] * 1000;

		// generate osculating vector 1P/2P format
		JniSpVec.SpVecFieldsToLines(pos, velMPS, 'U', xai_dcElts[JniBatchDC.XAI_DCELTS_SATNUM], "SPDC", SrvTimeFunc.UTCToYYYYDDDHHMMSSsssStr(xar_dcElts[JniBatchDC.XAR_DCELTS_EPOCHDS50UTC]),
				xai_dcElts[JniBatchDC.XAI_DCELTS_REVNUM], xai_dcElts[JniBatchDC.XAI_DCELTS_ELSETNUM], xar_dcElts[JniBatchDC.XAR_DCELTS_BFIELD], xar_dcElts[JniBatchDC.XAR_DCELTS_AGOM],
				xar_dcElts[JniBatchDC.XAR_DCELTS_OGPARM], "TMDAT", line1, line2);
	...
	{
		double[] xa_ac = new double[JniBatchDC.XA_AC_SIZE];

		JniBatchDC.BatchDCGetAccptCrit(satKey, xa_ac);
		...
		outFile.write(String.format("  STANDARD  %4.1f DAY    %3f     %3.0f   %4.0f KM   %5.1f    %6.3f DEG  %8.4f    %9.2e REV/DAY    %10.3e   m**2/kg\n",
				xa_ac[JniBatchDC.XA_AC_STD_EPOCH], xa_ac[JniBatchDC.XA_AC_STD_NORES], xa_ac[JniBatchDC.XA_AC_STD_PRCNTRES], xa_ac[JniBatchDC.XA_AC_STD_RMS],
				xa_ac[JniBatchDC.XA_AC_STD_OBSSPAN], xa_ac[JniBatchDC.XA_AC_STD_DELTAW], xa_ac[JniBatchDC.XA_AC_STD_DELTAABAR], xa_ac[JniBatchDC.XA_AC_STD_DELTAN],
				xa_ac[JniBatchDC.XA_AC_STD_DELTAB]));
		outFile.write(String.format("  ACTUAL    %4.1f DAY    %3f     %3.0f   %4d KM   %5.1f    %6.3f DEG  %8.4f    %9.2e REV/DAY    %10.3e   m**2/kg\n\n",
				xa_ac[JniBatchDC.XA_AC_ACT_EPOCH], xa_ac[JniBatchDC.XA_AC_ACT_NORES], xa_ac[JniBatchDC.XA_AC_ACT_PRCNTRES], (int)xa_ac[JniBatchDC.XA_AC_ACT_RMS],
				xa_ac[JniBatchDC.XA_AC_STD_OBSSPAN], xa_ac[JniBatchDC.XA_AC_ACT_DELTAW], xa_ac[JniBatchDC.XA_AC_STD_DELTAABAR], xa_ac[JniBatchDC.XA_AC_ACT_DELTAN], xa_ac[JniBatchDC.XA_AC_ACT_DELTAB]));
	}
	...
	public static void PrintObResiduals(BufferedWriter outFile, int[] xai_ctrl, double[] xar_ctrl, SrvObs.ObsRecord obsRecord, SrvRotas.Residuals residuals, int[] obsRej) throws IOException
	{
		...
		JniTimeFunc.UTCToDTG19(obsRecord.getObsTimeDs50UTC(), dtg);
		int obsType = residuals.getObsType();

		// no residual data avaialable due to error
		if (obsRej[JniBatchDC.XA_REJFLG_ERROR] != 0)
		{
			outFile.write(String.format(" %5d %3d %s  **** %s ****\n",
					...
					obRejErrStr[obsRej[JniBatchDC.XA_REJFLG_ERROR] - 1]));
			return;
		}


		if (xai_ctrl[JniBatchDC.XAI_CTRL_BRUCE] != 0 || obsRecord.getObsType() == 'P' || obsRecord.getObsType() == 'V')
			rejFlg = "NR";
		else if (obsRej[JniBatchDC.XA_REJFLG_ERROR] == 100)
			rejFlg = "XX";
		else if (obsRej[JniBatchDC.XA_REJFLG_RA] + obsRej[JniBatchDC.XA_REJFLG_DEC] + obsRej[JniBatchDC.XA_REJFLG_RANGE] + obsRej[JniBatchDC.XA_REJFLG_RR] > 0)
			rejFlg = String.format("*%1d", (obsRej[JniBatchDC.XA_REJFLG_RA] + obsRej[JniBatchDC.XA_REJFLG_DEC] + obsRej[JniBatchDC.XA_REJFLG_RANGE] + obsRej[JniBatchDC.XA_REJFLG_RR]));
		else
			rejFlg = "  ";
		...
		outFile.write(String.format(formatStr,
				...
				obsRej[JniBatchDC.XA_REJFLG_TIME] == 1 ? "*" : " ",						// 7. time rejection
				SrvDllMain.NonNegativeZero(residuals.getDeltaT(), 4),			// 8. delta t (min)
				obsRej[JniBatchDC.XA_REJFLG_BETA] == 1 ? "*" : " ",						// 9. delta beta rejection flag
				SrvDllMain.NonNegativeZero(residuals.getBeta(), 4),			//10. beta (deg)
				obsRej[JniBatchDC.XA_REJFLG_HEIGHT] == 1 ? "*" : " ",					//11. delta height rejection flag
				SrvDllMain.NonNegativeZero(residuals.getHeight(), 3),			//12. delta height (km)
				obsRej[JniBatchDC.XA_REJFLG_RANGE] == 1 ? "*" : " ",					//13. range residual rejection flag
				SrvDllMain.NonNegativeZero(residuals.getRange(), 3),			//14. range residual (km)
				obsRej[JniBatchDC.XA_REJFLG_RA] == 1 ? "*" : " ",						//15. azimuth or RA residual rejection flag
				SrvDllMain.NonNegativeZero(residuals.getRightAscension(), 3),	//16. azimuth or RA residual (deg)
				obsRej[JniBatchDC.XA_REJFLG_DEC] == 1 ? "*" : " ",						//17. elevation or declination residual rejection flag
				SrvDllMain.NonNegativeZero(residuals.getDeclination(), 3),		//18. elevation or declination residual (deg)
				obsRej[JniBatchDC.XA_REJFLG_RR] == 1 ? "*" : " ",						//19. range rate residual rejection flag
