...
/** This class defines the JNI component of the wrapper for: Bam */
public class JniBam
{
   // Provide the path to the dll/so
   // **************************************************************************
   /** The name of the DLL/SO that this JNI class wraps. */
   public static final String DLL_NAME = "bam";
   ...
   */
   public static native void LoadBamDll();
   ...
   * @return 0 if the Bam dll is initialized successfully, non-0 if there is an error
   */
   public static native int BamInit(long apAddr);


   /**
   * Returns information about the current version of Bam.dll. The information is placed in the string parameter you pass in
   * @param infoStr            A string to hold the information about Bam.dll (out-Character[128])
   */
   public static native void BamGetInfo(byte[] infoStr);
   ...
   * @param stepSizeMin        step size in minutes (in-Double)
   * @return number of time steps computed from the input time span and step size
   */
   public static native int BamCompNumTSs(double startDs50UTC, double stopDs50UTC, double stepSizeMin);
   ...
   * </table>
   * @param satKeys            array of satellite keys that wil be used in BAM (in-Long[*])
   * @param numSats            the size of the satKeys array (in-Integer)
   * @param startDs50UTC       start time in days since 1950, UTC (in-Double)
   * @param stopDs50UTC        stop time in days since 1950, UTC (in-Double)
   * @param stepSizeMin        step size in minutes (in-Double)
   * @param avgSDs             average separate distances of all time steps (out-Double[*])
   * @param avgMDs             average missed distances of all time steps (out-Double[*])
   * @param extBamArr          other BAM resulting data (out-Double[64])
   * @param errCode            0 if Bam is successful, non-0 if there is an error (out-Integer)
   */
   public static native void BamCompute(long[] satKeys, int numSats, double startDs50UTC, double stopDs50UTC, double stepSizeMin, double[] avgSDs, double[] avgMDs, double[] extBamArr, int[] errCode);
