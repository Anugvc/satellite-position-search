...
public class Sgp4Prop
{
   ...
   public static void main(String[] args) throws IOException
   {
      ...
      //--------------------------------------------------------------
      errCode = JnaDllMain.OpenLogFile(logFile);
      if (errCode != 0)
         SrvDllMain.ShowMsgAndTerminate();

      // Load Tles from the input file
      errCode = JnaTle.TleLoadFile(inFile);
      if (errCode != 0)
         SrvDllMain.ShowMsgAndTerminate();

      // Load 6P-Card
      errCode = JnaTimeFunc.TConLoadFile(inFile);
      if (errCode != 0)
         SrvDllMain.ShowMsgAndTerminate();
      ...
      JnaTle.TleGetLoaded(2, satKeys);  // Get the satKeys in the order they were read

      // Get information about the current DLL
      JnaSgp4Prop.Sgp4GetInfo(sgp4DllInfo);
      ...
      for (int i = 0; i < numSats; i++)
      {
         ...
         errCode = JnaTle.TleGetLines(satKey, l1, l2);
         if (errCode != 0)
         {
         ...
         JnaTle.TleGetField(satKey, JnaTle.XF_TLE_EPOCH, valueStr);

         double epochDs50UTC  = JnaTimeFunc.DTGToUTC(new String(valueStr));
         ...
         for (int step = 0; true; step++)
         {
            ...
            errCode = JnaSgp4Prop.Sgp4PropDs50UTC(satKey, ds50UTC, mse, pos, vel, llh);
            if (errCode != 0)
            {
            ...
            //----------------------------------------------------------------
            JnaSgp4Prop.Sgp4GetPropOut(satKey, JnaSgp4Prop.XF_SGP4OUT_OSC_KEP, oscKep);
            JnaSgp4Prop.Sgp4GetPropOut(satKey, JnaSgp4Prop.XF_SGP4OUT_MEAN_KEP, meanKep);
            JnaSgp4Prop.Sgp4GetPropOut(satKey, JnaSgp4Prop.XF_SGP4OUT_NODAL_AP_PER, nodalApPer);


            // Print position and velocity
            PrintPosVel(swOscState, mse.getValue(), pos, vel);

            // Print osculating Keplerian elements
            PrintOscEls(swOscElem, mse.getValue(), oscKep);

            // Print mean Keplerian elements
            PrintMeanEls(swMeanElem, mse.getValue(), meanKep);

            // Print lat lon height and position
            PrintLLH(swLLH, mse.getValue(), llh, pos);

            // Print nodal period, apogee, perigee, and others
            meanMotion = JnaAstroFunc.AToN(meanKep[0]);
            PrintNodalApPer(swNodalApPer, mse.getValue(), meanMotion, nodalApPer);
         ...
         if (JnaSgp4Prop.Sgp4RemoveSat(satKey) != 0)
            SrvDllMain.ShowMsgAndTerminate();
   ...
   {
      double trueAnomaly = JnaAstroFunc.CompTrueAnomaly(oscKep);
   ...
   {
      double meanMotion = JnaAstroFunc.AToN(meanKep[0]);
   ...
   {
      IntByReference startFrEpoch = new IntByReference();
      IntByReference stopFrEpoch  = new IntByReference();
      DoubleByReference startTime  = new DoubleByReference();
      DoubleByReference stopTime  = new DoubleByReference();
      DoubleByReference stepSize  = new DoubleByReference();

      // Get prediction control data
      JnaTimeFunc.Get6P(startFrEpoch, stopFrEpoch, startTime, stopTime, stepSize);

      // Compute start/stop times - using days since 1950 UTC
      // user selects start time in minutes since epoch
      if (startFrEpoch.getValue() == 1)
         timeArr[0] = epoch + (startTime.getValue() / 1440);
      else // user selects start time in days since 1950 UTC
         timeArr[0] = startTime.getValue();

      // user selects stop time in minutes since epoch
      if (stopFrEpoch.getValue() == 1)
         timeArr[1] = epoch + (stopTime.getValue() / 1440);
      else // user selects stop time in days since 1950 UTC
         timeArr[1] = stopTime.getValue();

      if(timeArr[0] > timeArr[1])
         timeArr[2] = -Math.abs(stepSize.getValue());
      else
         timeArr[2] = Math.abs(stepSize.getValue());
   }
}
