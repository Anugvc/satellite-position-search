...
public class SrvTle
{
	...
	public static double ComputePerigeeHt(TleGPRecord tle)
	{
		...
		perigeeHt = (JnaAstroFunc.NToA(tle.getMnMotion()) * (1 - tle.getEccen())) - JnaEnvConst.EnvGetGeoConst(JnaEnvConst.XF_GEOCON_KMPER);
	...
	public static class TleGPRecord
	{
		...
		public TleGPRecord(String line1, String line2)
		{
			...
			IntByReference     epochYr      = new IntByReference();
			DoubleByReference  epochDays    = new DoubleByReference();
			DoubleByReference  bstar        = new DoubleByReference();
			...
			DoubleByReference  nDotO2       = new DoubleByReference();
			DoubleByReference  n2DotO6      = new DoubleByReference();

			JnaTle.TleParseGP(line1, line2, satNum, secClass, satName, epochYr, epochDays, nDotO2, n2DotO6,
			bstar, ephType, elsetNum, incli, node, eccen, omega, mnAnomaly, mnMotion, revNum);

			this.satNum = satNum.getValue();
			this.secClass = (char)secClass.getValue();
			this.satName = Utility.BytesToStringTrim(satName);
			this.epochYr = epochYr.getValue();
			this.epochDays = epochDays.getValue();
			this.nDotO2 = nDotO2.getValue();
			this.n2DotO6 = n2DotO6.getValue();
			this.bstar = bstar.getValue();
			this.ephType = ephType.getValue();
			this.elsetNum = elsetNum.getValue();
			this.incli = incli.getValue();
			this.node = node.getValue();
			this.eccen = eccen.getValue();
			this.omega = omega.getValue();
			this.mnAnomaly = mnAnomaly.getValue();
			this.mnMotion = mnMotion.getValue();
			this.revNum = revNum.getValue();
			this.epochDs50UTC = JnaTimeFunc.YrDaysToUTC(epochYr.getValue(), epochDays.getValue());
		}
