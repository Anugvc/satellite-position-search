...
public class SrvAof {
	 ...
	 * @param xa_aofCtrl	 AOF control parameters, see XF_AOFCTRL_? (JnaAof.java) for array arrangement
	 */
	 ...
	 * @param fpOut			The BufferedWriter object carrying the file to print to
	 * @param xa_aofArea	defended area data, see XA_AOFAREA_? (JnaAof.java) for array arrangement
	 */
	public static void PrintAreaData(BufferedWriter fpOut, double[] xa_aofArea)
	{
		...
		try {
			...
			fpOut.write(String.format(" |                           DEFENDED AREA %4d DATA                           |\n", (int)xa_aofArea[JnaAof.XA_AOFAREA_NUM]));
			fpOut.write(String.format(" %s\n", dashLine));

			areaType = (int)xa_aofArea[JnaAof.XA_AOFAREA_TYPE];

			fpOut.write(String.format(" |  Area type:   %-62s|\n", areaTypeStrs[areaType - 1]));
			fpOut.write(String.format(" |  Latitude :   %8.4f deg N%48s|\n", xa_aofArea[JnaAof.XA_AOFAREA_ELEM1], " "));
			fpOut.write(String.format(" |  Longitude:   %8.4f deg E%48s|\n", xa_aofArea[JnaAof.XA_AOFAREA_ELEM2], " "));
			if (areaType == JnaAof.AOF_AREATYPE_BOX)
			{
				fpOut.write(String.format(" |  Latitude :   %8.4f deg N%48s|\n", xa_aofArea[JnaAof.XA_AOFAREA_ELEM3], " "));
				fpOut.write(String.format(" |  Longitude:   %8.4f deg E%48s|\n", xa_aofArea[JnaAof.XA_AOFAREA_ELEM4], " "));
			}
			else if (areaType == JnaAof.AOF_AREATYPE_POINT)
			{
	...
	public static void PrintEntryExit(BufferedWriter fpOut, double[] xa_aofArea, int satNum, long satKey, int numOfPasses, double[] xa_entExitTimes)
	{
		...
		printLook = xa_aofArea[JnaAof.XA_AOFAREA_TYPE] == JnaAof.AOF_AREATYPE_CIRCLE || xa_aofArea[JnaAof.XA_AOFAREA_TYPE] == JnaAof.AOF_AREATYPE_POINT;

		try {
			fpOut.write(String.format("\n   Satellite no.    %05d                    Area no.     %04d\n", satNum, (int) xa_aofArea[JnaAof.XA_AOFAREA_NUM]));

			for (i = 0; i < numOfPasses; i++) {
				for (j = 0; j < 2; j++) {
					darkness = JnaAof.AofGetDarknessLevel(xa_entExitTimes[i * 2 + j], xa_aofArea);
					fpOut.write(String.format(" SAT %s AREA AT %s  %-18s    ", enterExitStrs[j], SrvTimeFunc.UTCToDtg20Str(JnaTimeFunc.TAIToUTC(xa_entExitTimes[i * 2 + j])), darknessStrs[darkness]));

					if (printLook) {
						llh[0] = xa_aofArea[JnaAof.XA_AOFAREA_ELEM1];
						llh[1] = xa_aofArea[JnaAof.XA_AOFAREA_ELEM2];
						llh[2] = xa_aofArea[JnaAof.XA_AOFAREA_ELEM3];

						// compute look angle from the location llh to the overfly satellite at the specified time
						JnaAof.AofGetLook(xa_entExitTimes[i * 2 + j], llh, satKey, xa_look);

						fpOut.write(String.format("%05d  AZ= %5.1f  EL= %4.1f  RNG= %5.0f  (km)\n", satNum, xa_look[JnaLamod.XA_LOOK_AZIM], xa_look[JnaLamod.XA_LOOK_ELEV], xa_look[JnaLamod.XA_LOOK_RNG]));
					} else {
						fpOut.write("\n");
					}
				}
			}
			fpOut.write(String.format(" Satellite %5d: %3d pass(es) over defended area %4d\n", satNum, numOfPasses, (int) xa_aofArea[JnaAof.XA_AOFAREA_NUM]));
			fpOut.write("\n\n");
		} catch(Exception e){
			e.printStackTrace();
