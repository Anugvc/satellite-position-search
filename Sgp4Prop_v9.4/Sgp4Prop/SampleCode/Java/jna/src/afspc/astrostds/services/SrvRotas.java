...
public class SrvRotas
{
	...
	public static void PrintNoMatchingElset(BufferedWriter fpOut, SrvObs.ObsRecord obs, int obsNum, boolean useNewFormat) throws IOException
	{
		...
		JnaDllMain.GetLastErrMsg(errMsg);
		errMsg[78] = 0;
	...
	public static class Residuals
	{
		...
		{
			double[] obsResArr = new double[JnaRotas.XA_OBSRES_SIZE];
			double[] satElts = new double[9];
			double[] obElts = new double[9];
			ErrCode = JnaRotas.RotasComputeObsResiduals(obsKey, satKey, obsResArr, satElts, obElts);
		...
		{
			ASTAT              = (int)resArr[JnaRotas.XA_OBSRES_ASTAT];
			if (ErrCode != 0 || ASTAT > 4)
			{
				return;
			}

			Azimuth            = resArr[JnaRotas.XA_OBSRES_AZ];
			Elevation          = resArr[JnaRotas.XA_OBSRES_EL];
			RightAscension     = resArr[JnaRotas.XA_OBSRES_RA];
			Declination        = resArr[JnaRotas.XA_OBSRES_DEC];
			Range              = resArr[JnaRotas.XA_OBSRES_RANGE];
			Height             = resArr[JnaRotas.XA_OBSRES_HEIGHT];
			Beta               = resArr[JnaRotas.XA_OBSRES_BETA];
			DeltaT             = resArr[JnaRotas.XA_OBSRES_DELTAT];
			VMag               = resArr[JnaRotas.XA_OBSRES_VMAG];
			Age                = resArr[JnaRotas.XA_OBSRES_AGE];
			SatArgOfLatitude   = resArr[JnaRotas.XA_OBSRES_SU];
			RevNum             = resArr[JnaRotas.XA_OBSRES_REVNUM];
			RangeRate          = resArr[JnaRotas.XA_OBSRES_RNGRATE];
			SatTrueAnomaly     = resArr[JnaRotas.XA_OBSRES_SATANOM];
			SatElevation       = resArr[JnaRotas.XA_OBSRES_SATELEV];
			ObsType            = (int)resArr[JnaRotas.XA_OBSRES_OBSTYPE];
			SatCat             = (int)resArr[JnaRotas.XA_OBSRES_SATCAT];

			ObsKey             = obsKey;
			SatKey             = satKey;
			ObsTimeDs50UTC     = resArr[JnaRotas.XA_OBSRES_OBSTIME];

			VelAngle           = resArr[JnaRotas.XA_OBSRES_VELANG];
			AngularMomentum    = resArr[JnaRotas.XA_OBSRES_ANGMOM];
		}
		...
		public void PrintObsResiduals(BufferedWriter fpOut, SrvObs.ObsRecord obs, int obsNum, boolean useNewFormat) throws IOException
		{
			...
			if (propType == JnaDllMain.PROPTYPE_GP)
				propTypeChar = 'G';
			else if (propType == JnaDllMain.PROPTYPE_SP)
				propTypeChar = 'S';
			else if (propType == JnaDllMain.PROPTYPE_X)
				propTypeChar = 'X';
			else
				propTypeChar = ' ';
	...
	public static class RotasControl
	{
		...
		public RotasControl() {
			...
			DoubleByReference grossBeta = new DoubleByReference();
			DoubleByReference betaLimit = new DoubleByReference();
			DoubleByReference deltaTLimit = new DoubleByReference();
			DoubleByReference deltaHLimit = new DoubleByReference();
			IntByReference astat2Mult = new IntByReference();
			IntByReference prtFlag = new IntByReference();
			IntByReference retagFlag = new IntByReference();
			IntByReference ltcFlag = new IntByReference();
			IntByReference maxNumAssoc = new IntByReference();
			IntByReference debiasFlag = new IntByReference();
			IntByReference diagMode = new IntByReference();
			IntByReference covPrtFlag = new IntByReference();
			IntByReference isTrackMode = new IntByReference();
			IntByReference remRetagObs = new IntByReference();

			double[] extArr = new double[128];
			JnaRotas.RotasGetPAll(rotasMode,
				grossBeta,
                betaLimit,
                deltaTLimit,
                deltaHLimit,
			...
			rotasMode[5] = 0;
			setGrossBeta(grossBeta.getValue());
			setBetaLimit(betaLimit.getValue());
			setDeltaTLimit(deltaTLimit.getValue());
			setDeltaHLimit(deltaHLimit.getValue());
			setAstat2Mult(astat2Mult.getValue());
			setPrtFlag(prtFlag.getValue());
			setRetagFlag(retagFlag.getValue());
			setLtcFlag(ltcFlag.getValue());
			setMaxNumAssoc(maxNumAssoc.getValue());
			setDebiasFlag(debiasFlag.getValue());
			setDiagMode(diagMode.getValue());
			setCovPrtFlag(covPrtFlag.getValue());
			setIsTrackMode(isTrackMode.getValue());
			setRemRetagObs(remRetagObs.getValue());
		}
		...
		public void PrintAssocMultipliers(BufferedWriter fpOut) throws IOException
		{
			...
			JnaRotas.RotasGetAssocMultipliers(assocMultArr);
		...
		public void DoRetagObs(BufferedWriter fpOut, long obsKey, int newTag, int ASTAT) throws IOException
		{
			...
			if(retagFlag == RETAG_B3_ASTAT0_1 || retagFlag == RETAG_B3_ASTAT0_2)
			{
				...
				JnaObs.ObsGetB3Card(obsKey, b3Card);
			...
			else if(retagFlag == RETAG_TTY_ASTAT0_1)
			{
				...
				JnaObs.ObsGetTTYCard(obsKey, ttyCard1, ttyCard2);
				ttyCard1[90] = 0;
				ttyCard2[90] = 0;
