...
public class SrvFov
{
	...
	public static void PrintFovCtrls(BufferedWriter fpOut, double[] xa_fovCtrl, int numOfWindows, double[] startStopTimes,
	   ...
	   if (numOfSources == 1)
	   {
		  id = (int)xa_fovSrc_2D[0][JnaFov.XA_FOVSRC_ID];
		  srcType = (int)xa_fovSrc_2D[0][JnaFov.XA_FOVSRC_TYPE];
	   ...
	   else
	   {
		  ...
		  {
			 id = (int)xa_fovSrc_2D[i][JnaFov.XA_FOVSRC_ID];
			 srcType = (int)xa_fovSrc_2D[i][JnaFov.XA_FOVSRC_TYPE];
	   ...
	   if (numOfTargets == 1)
	   {
		  id = (int)xa_fovTgt_2D[0][JnaFov.XA_FOVTGT_ID];
		  tgtType = (int)xa_fovTgt_2D[0][JnaFov.XA_FOVTGT_TYPE];
	   ...
	   else
	   {
		  ...
		  {
			 id = (int)xa_fovTgt_2D[i][JnaFov.XA_FOVTGT_ID];
			 tgtType = (int)xa_fovTgt_2D[i][JnaFov.XA_FOVTGT_TYPE];
	...
	public static void PrintSourceTargetData(BufferedWriter fpOut, double[] xa_fovSrc, double[] xa_fovTgt) throws IOException {
	   ...
	   tgtId = (int)xa_fovTgt[JnaFov.XA_FOVTGT_ID];
	   tgtType = (int)xa_fovTgt[JnaFov.XA_FOVTGT_TYPE];
	   srcId = (int)xa_fovSrc[JnaFov.XA_FOVSRC_ID];
	   srcType = (int)xa_fovSrc[JnaFov.XA_FOVSRC_TYPE];
	...
	public static void PrintPenetrationTimes(BufferedWriter fpOut, boolean isNewWindow, int winNum, double[] xa_fovSrc, double[] xa_fovTgt, long vicSatKey, double[][] xa_emeDat, int numOfPens) throws IOException {
	   ...
	   for (m = 0; m < numOfPens; m++)
	   {
		  ...
		  fpOut.write(String.format(" SAT = %5d ENTRY TIME = %s AZ =%6.1f EL =%5.1f RG =%8.1f RR =%7.3f\n", 
			 JnaElOps.SatNumOf(vicSatKey),
			 SrvTimeFunc.UTCToDtg20Str(xa_look[JnaLamod.XA_LOOK_DS50UTC]),
			 xa_look[JnaLamod.XA_LOOK_AZIM],
			 xa_look[JnaLamod.XA_LOOK_ELEV],
			 xa_look[JnaLamod.XA_LOOK_RNG],
			 xa_look[JnaLamod.XA_LOOK_RNGRT]));
	...
	public static void PrintDetailLookAngles(BufferedWriter fpOut, double[] xa_fovRun, double[] xa_fovSrc, double[] xa_fovTgt, long tgtSatKey, long vicSatKey) throws IOException {
	   ...
	   startTimeDs50UTC = xa_fovRun[JnaFov.XA_FOVRUN_START];
	   stopTimeDs50UTC = xa_fovRun[JnaFov.XA_FOVRUN_STOP];
	   interval = xa_fovRun[JnaFov.XA_FOVRUN_INTERVAL];

	   // search interval depends on target satellite's period
	   if (interval == 0)
	   {
		  period_min = SrvSatState.GetSatDataField(tgtSatKey, JnaSatState.XF_SAT_PERIOD);
		  if (period_min < 225)
	   ...
	   for (m = 0; currDs50UTC <= stopTimeDs50UTC; m++)
	   {
		  ...
		  currDs50TAI = JnaTimeFunc.UTCToTAI(currDs50UTC);
		  ...
		  fpOut.write(String.format(" %s %9.3f%9.3f%11.2f%9.3f%9.3f%9.3f%11.2f%9.3f\n",
			 SrvTimeFunc.UTCToDtg20Str(xa_look_tgt[JnaLamod.XA_LOOK_DS50UTC]),
			 xa_look_tgt[JnaLamod.XA_LOOK_AZIM],
			 xa_look_tgt[JnaLamod.XA_LOOK_ELEV],
			 xa_look_tgt[JnaLamod.XA_LOOK_RNG],
			 xa_look_tgt[JnaLamod.XA_LOOK_RNGRT],
			 xa_look_vic[JnaLamod.XA_LOOK_AZIM],
			 xa_look_vic[JnaLamod.XA_LOOK_ELEV],
			 xa_look_vic[JnaLamod.XA_LOOK_RNG],
			 xa_look_vic[JnaLamod.XA_LOOK_RNGRT]));
	   }
	}
	...
	public static void GetLookAngle(int tgtType, double ds50TAI, double[] xa_fovSrc, double[] xa_fovTgt, long vicSatKey, double[] xa_look)
	{
	   ...
	   if (tgtType == JnaFov.FOV_TGTTYPE_ELSET)
		  errCode = JnaFov.FovTargetElsetLook(ds50TAI, xa_fovSrc, vicSatKey, xa_look);
	   else
		  errCode = JnaFov.FovTargetVecLook(ds50TAI, xa_fovSrc, xa_fovTgt, vicSatKey, xa_look);
	}
}