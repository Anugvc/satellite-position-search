-- This wrapper file was generated automatically by the GenDllWrappers program.

pragma Ada_2012;
pragma Style_Checks (Off);
pragma Warnings ("U");
with AstroAdaTypes; use AstroAdaTypes;

package SpVecDll is
    
    -- Indexes of SPVEC data fields
    -- X component of satellite's position (km)
    XF_SPVEC_POS1 : constant := 1;
    -- Y component of satellite's position (km)
    XF_SPVEC_POS2 : constant := 2;
    -- Z component of satellite's position (km)
    XF_SPVEC_POS3 : constant := 3;
    -- X component of satellite's velocity (m/s)
    XF_SPVEC_VEL1 : constant := 4;
    -- Y component of satellite's velocity (m/s)
    XF_SPVEC_VEL2 : constant := 5;
    -- Z component of satellite's velocity (m/s)
    XF_SPVEC_VEL3 : constant := 6;
    -- Security classification
    XF_SPVEC_SECCLASS : constant := 7;
    -- Satellite number
    XF_SPVEC_SATNUM : constant := 9;
    -- Satellite common name
    XF_SPVEC_SATNAME : constant := 10;
    -- Epoch date
    XF_SPVEC_EPOCH : constant := 11;
    -- Epoch revolution number
    XF_SPVEC_REVNUM : constant := 12;
    -- Elset number
    XF_SPVEC_ELSETNUM : constant := 13;
    -- Ballistic coefficient (m^2/kg)
    XF_SPVEC_BTERM : constant := 14;
    -- Radiation pressure coefficient (m^2/kg)
    XF_SPVEC_AGOM : constant := 15;
    -- Outgassing parameter (km/s^2)
    XF_SPVEC_OGPARM : constant := 16;
    -- Inpute coordinate system
    XF_SPVEC_INPCOORD : constant := 17;
    
    
    -- Indexes of SPVEC numerical data in an array
    -- Satellite number
    XA_SPVEC_SATNUM : constant := 0;
    -- Epoch date in days since 1950 UTC
    XA_SPVEC_EPOCH : constant := 1;
    -- Epoch revolution number
    XA_SPVEC_REVNUM : constant := 2;
    -- Elset number
    XA_SPVEC_ELSETNUM : constant := 3;
    -- Ballistic coefficient (m^2/kg)
    XA_SPVEC_BTERM : constant := 4;
    -- Radiation pressure coefficient (m^2/kg)
    XA_SPVEC_AGOM : constant := 5;
    -- Outgassing parameter (km/s^2)
    XA_SPVEC_OGPARM : constant := 6;
    -- Inpute coordinate systemm; see SPVEC_INPCOORD_? for available options
    XA_SPVEC_INPCOORD : constant := 7;
    
    -- X component of satellite's position (km)
    XA_SPVEC_POS1 : constant := 20;
    -- Y component of satellite's position (km)
    XA_SPVEC_POS2 : constant := 21;
    -- Z component of satellite's position (km)
    XA_SPVEC_POS3 : constant := 22;
    -- X component of satellite's velocity (m/s)
    XA_SPVEC_VEL1 : constant := 23;
    -- Y component of satellite's velocity (m/s)
    XA_SPVEC_VEL2 : constant := 24;
    -- Z component of satellite's velocity (m/s)
    XA_SPVEC_VEL3 : constant := 25;
    
    -- Flag to indicate SP vec has its own numerical integration control (vs global 4P settings)
    XA_SPVEC_HASOWNCRL : constant := 70;
    -- Geopotential model to use
    XA_SPVEC_GEOIDX : constant := 71;
    -- Earth gravity pertubations flag
    XA_SPVEC_BULGEFLG : constant := 72;
    -- Drag pertubations flag
    XA_SPVEC_DRAGFLG : constant := 73;
    -- Radiation pressure pertubations flag
    XA_SPVEC_RADFLG : constant := 74;
    -- Lunar/Solar pertubations flag
    XA_SPVEC_LUNSOLFLG : constant := 75;
    -- F10 value
    XA_SPVEC_F10 : constant := 76;
    -- F10 average value
    XA_SPVEC_F10AVG : constant := 77;
    -- Ap value
    XA_SPVEC_AP : constant := 78;
    -- Geopotential truncation order/degree/zonals
    XA_SPVEC_TRUNC : constant := 79;
    -- Corrector step convergence criterion; exponent of 1/10; default = 10
    XA_SPVEC_CONVERG : constant := 80;
    -- Outgassing pertubations flag
    XA_SPVEC_OGFLG : constant := 81;
    -- Solid earth and ocean tide pertubations flag
    XA_SPVEC_TIDESFLG : constant := 82;
    -- Nutation terms
    XA_SPVEC_NTERMS : constant := 84;
    -- Recompute pertubations at each corrector step
    XA_SPVEC_REEVAL : constant := 85;
    -- Variable of intergration control
    XA_SPVEC_INTEGCTRL : constant := 86;
    -- Variable step size control
    XA_SPVEC_VARSTEP : constant := 87;
    -- Initial step size
    XA_SPVEC_INITSTEP : constant := 88;
    
    -- weighted RM of last DC on the satellite
    XA_SPVEC_RMS : constant := 99;
    -- the lower triangle portion of the full cov matrix (100-120: 6x6 covmtx, ..., 100-154: 10x10 covmtx)
    XA_SPVEC_COVELEMS : constant := 100;
    
    XA_SPVEC_SIZE : constant := 512;
    
    
    -- Indexes of SPVEC text data in an array of chars
    -- Security classification
    XS_SPVEC_SECCLASS_1 : constant := 0;
    -- Satellite common name
    XS_SPVEC_SATNAME_8 : constant := 1;
    
    XS_SPVEC_SIZE : constant := 512;
    
    -- SPVEC's text data fields - new convention (start index, string length)
    -- Security classification
    XS_SPVEC_SECCLASS_0_1 : constant := 0;
    -- Satellite common name
    XS_SPVEC_SATNAME_1_8 : constant := 1;
    
    XS_SPVEC_LENGTH : constant := 512;
    
    
    -- Different input coordinate for SpVec
    -- Use input coordinate specified in 4P-card
    SPVEC_INPCOORD_4P : constant := 0;
    -- Input coordinate systems is coordinates of epoch
    SPVEC_INPCOORD_TMDAT : constant := 1;
    -- Input coordinate systems is coordinates of J2000
    SPVEC_INPCOORD_MMJ2K : constant := 2;
    

    -- Notes: This function has been deprecated since v9.0. 
    -- Initializes SpVec DLL for use in the program
    -- If this function returns an error, it is recommended that the users stop the program immediately. The error occurs if the users forget to load and initialize all the prerequisite DLLs, as listed in the DLL Prerequisites section, before using this DLL.
    function SpVecInit
        (apAddr            :         Long_Long_Integer) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecInit";

    -- Returns information about the current version of SpVec DLL. The information is placed in the string parameter you pass in
    -- The returned string provides information about the version number, build date, and the platform of the SpVec DLL. 
    procedure SpVecGetInfo
        (infoStr           : out     String)
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetInfo";
        --infoStr            : String    (1..128);

    -- Loads a text file containing SpVec's
    -- The users can use this function repeatedly to load B1P records from different input files. However, only unique satKeys are stored in the binary tree. Duplicated satKeys won't be stored. 
    -- 
    -- B1P records can be included directly in the main input file or they can be read from a separate file identified with "ELTFIL=[pathname\filename]" or "VECFIL=[path/filename]".
    -- 
    -- This function only reads B1P records from the main input file or B1P records from the file identified with ELTFIL or VECFIL in the input file. It won't read anything else.
    function SpVecLoadFile
        (spVecFile         : in      String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecLoadFile";
        --spVecFile          : String    (1..512);

    -- Saves the currently loaded SpVecs's to a file
    -- If the users call this routine immediately after SpVecLoadFile. The SPVECs contents in the two file should be the same (minus duplicated SPVECs or bad SPVECs).
    -- 
    -- The purpose of this function is to save the current state of the loaded SPVECs, usually used in GUI applications, for future use.
    function SpVecSaveFile
        (spVecFile         : in      String;
        saveMode           :         Integer;
        saveForm           :         Integer) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecSaveFile";
        --spVecFile          : String    (1..512);

    -- Removes an SpVec represented by the satKey from memory
    -- If the users enter an invalid satKey, a non-existing satKey in memory, the function will return a non-zero value indicating an error.
    function SpVecRemoveSat
        (satKey            :         Long_Long_Integer) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecRemoveSat";

    -- Removes all SpVec's from memory
    function SpVecRemoveAllSats return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecRemoveAllSats";

    -- Retrieves all of the currently loaded satKeys. These satKeys can be used to access the internal data for the SpVec's
    -- See SpVecGetLoaded for example.
    -- This function is useful for dynamically allocating memory for the array that is passed to the function SpVecGetLoaded().
    function SpVecGetCount return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetCount";

    -- Retrieves all of the currently loaded satKeys. These satKeys can be used to access the internal data for the SpVec's
    -- It is recommended that SpVecGetCount() is used to count how many satellites are currently loaded in the SpVec DLL's binary tree. The users then use this number to dynamically allocate the satKeys array and pass it to this function. 
    -- 
    -- If the users prefer to pass a static array to the function, make sure it is big enough to store all the satKeys in memory.
    procedure SpVecGetLoaded
        (order             :         Integer;
        satKeys            : out     Long1D_Ptr)
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetLoaded";

    -- Adds an SpVec using its directly specified first and second lines
    -- If the satellite was previously added to the SpVec DLL's binary tree, the function also returns a negative value indicating an error. 
    -- 
    -- The users can use this function repeatedly to add many satellites (one satellite at a time) to the SpVec DLL's binary tree. 
    function SpVecAddSatFrLines
        (line1             : in      String;
        line2              : in      String) return Long_Long_Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecAddSatFrLines";
        --line1              : String    (1..512);
        --line2              : String    (1..512);

    -- Works like SpVecAddSatFrLines but designed for Matlab
    procedure SpVecAddSatFrLinesML
        (line1             : in      String;
        line2              : in      String;
        satKey             : out     Long_Long_Integer)
    with Import => True,
        Convention => C,
        External_Name => "SpVecAddSatFrLinesML";
        --line1              : String    (1..512);
        --line2              : String    (1..512);

    -- Adds an SpVec using its individually provided field values
    function SpVecAddSatFrFields
        (pos               : in      Double1D;
        vel                : in      Double1D;
        secClass           :         Character;
        satNum             :         Integer;
        satName            : in      String;
        epochDtg           : in      String;
        revNum             :         Integer;
        elsetNum           :         Integer;
        bterm              :         Long_Float;
        agom               :         Long_Float;
        ogParm             :         Long_Float;
        coordSys           : in      String) return Long_Long_Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecAddSatFrFields";
        --pos                : Double1D  (1..3);
        --vel                : Double1D  (1..3);
        --satName            : String    (1..8);
        --epochDtg           : String    (1..17);
        --coordSys           : String    (1..5);

    -- Works like SpVecAddSatFrFields but designed for Matlab 
    procedure SpVecAddSatFrFieldsML
        (pos               : in      Double1D;
        vel                : in      Double1D;
        secClass           :         Character;
        satNum             :         Integer;
        satName            : in      String;
        epochDtg           : in      String;
        revNum             :         Integer;
        elsetNum           :         Integer;
        bterm              :         Long_Float;
        agom               :         Long_Float;
        ogParm             :         Long_Float;
        coordSys           : in      String;
        satKey             : out     Long_Long_Integer)
    with Import => True,
        Convention => C,
        External_Name => "SpVecAddSatFrFieldsML";
        --pos                : Double1D  (1..3);
        --vel                : Double1D  (1..3);
        --satName            : String    (1..8);
        --epochDtg           : String    (1..17);
        --coordSys           : String    (1..5);

    -- Updates an SpVec satellite's data in memory using individually provided field values. Note: satNum, epoch string can't be updated.
    -- The satellite's unique key will not be changed in this function call. 
    function SpVecUpdateSatFrFields
        (satKey            :         Long_Long_Integer;
        pos                : in      Double1D;
        vel                : in      Double1D;
        secClass           :         Character;
        satName            : in      String;
        revNum             :         Integer;
        elsetNum           :         Integer;
        bterm              :         Long_Float;
        agom               :         Long_Float;
        ogParm             :         Long_Float;
        coordSys           : in      String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecUpdateSatFrFields";
        --pos                : Double1D  (1..3);
        --vel                : Double1D  (1..3);
        --satName            : String    (1..8);
        --coordSys           : String    (1..5);

    -- Retrieves the value of a specific field of an SpVec
    -- 
    -- The table below shows the values for the xf_SpVec parameter:
    -- 
    -- table
    -- 
    -- Index
    -- Index Interpretation
    -- 
    --  1-3First 3 elements of 1P
    --  4-6Second 3 elements of 1P
    --  7Security classification
    --  9Satellite number
    -- 10Satellite common name
    -- 11Epoch date
    -- 12Epoch revolution number
    -- 13Elset number
    -- 14Ballistic coefficient
    -- 15Radiation pressure coefficient
    -- 16Outgassing parameter
    -- 17Input coordinate system
    -- 
    function SpVecGetField
        (satKey            :         Long_Long_Integer;
        xf_SpVec           :         Integer;
        valueStr           : out     String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetField";
        --valueStr           : String    (1..512);

    -- Updates the value of a field of an SpVec
    -- See SpVecGetField for a description of the xf_SpVec parameter.  satNum (9) and epoch date (11) cannot be altered.
    -- The set value type was intentionally chosen as a character string because it allows the users to set value for different data types.
    function SpVecSetField
        (satKey            :         Long_Long_Integer;
        xf_SpVec           :         Integer;
        valueStr           : in      String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecSetField";
        --valueStr           : String    (1..512);

    -- Retrieves all of the data for an SpVec satellite in a single function call
    function SpVecGetAllFields
        (satKey            :         Long_Long_Integer;
        pos                : out     Double1D;
        vel                : out     Double1D;
        secClass           : out     Character;
        satNum             : out     Integer;
        satName            : out     String;
        epochDtg           : out     String;
        revNum             : out     Integer;
        elsetNum           : out     Integer;
        bterm              : out     Long_Float;
        agom               : out     Long_Float;
        ogParm             : out     Long_Float;
        coordSys           : out     String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetAllFields";
        --pos                : Double1D  (1..3);
        --vel                : Double1D  (1..3);
        --satName            : String    (1..8);
        --epochDtg           : String    (1..17);
        --coordSys           : String    (1..5);

    -- Retrieves all of the data for an SpVec satellite in a single function call
    function SpVecParse
        (line1             : in      String;
        line2              : in      String;
        pos                : out     Double1D;
        vel                : out     Double1D;
        secClass           : out     Character;
        satNum             : out     Integer;
        satName            : out     String;
        epochDtg           : out     String;
        revNum             : out     Integer;
        elsetNum           : out     Integer;
        bterm              : out     Long_Float;
        agom               : out     Long_Float;
        ogParm             : out     Long_Float;
        coordSys           : out     String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecParse";
        --line1              : String    (1..512);
        --line2              : String    (1..512);
        --pos                : Double1D  (1..3);
        --vel                : Double1D  (1..3);
        --satName            : String    (1..8);
        --epochDtg           : String    (1..17);
        --coordSys           : String    (1..5);

    -- Parses SPVEC data from the input first and second lines of an 1P/2P state vector and store that data back into the output parameters.
    -- This function only parses data from the input SPVEC but DOES NOT load/add the input SPVEC to memory.
    function SpVecLinesToArray
        (line1             : in      String;
        line2              : in      String;
        xa_spVec           : out     Double1D;
        xs_spVec           : out     String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecLinesToArray";
        --line1              : String    (1..512);
        --line2              : String    (1..512);
        --xa_spVec           : Double1D  (1..512);
        --xs_spVec           : String    (1..512);

    -- Returns the first and second lines of the 1P/2P representation of an SpVec
    function SpVecGetLines
        (satKey            :         Long_Long_Integer;
        line1              : out     String;
        line2              : out     String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetLines";
        --line1              : String    (1..512);
        --line2              : String    (1..512);

    -- Constructs 1P/2P cards from individually provided SpVec data fields
    -- Returned line1 and line2 are empty if the function fails to construct the lines as requested.
    procedure SpVecFieldsToLines
        (pos               : in      Double1D;
        vel                : in      Double1D;
        secClass           :         Character;
        satNum             :         Integer;
        satName            : in      String;
        epochDtg           : in      String;
        revNum             :         Integer;
        elsetNum           :         Integer;
        bterm              :         Long_Float;
        agom               :         Long_Float;
        ogParm             :         Long_Float;
        coordSys           : in      String;
        line1              : out     String;
        line2              : out     String)
    with Import => True,
        Convention => C,
        External_Name => "SpVecFieldsToLines";
        --pos                : Double1D  (1..3);
        --vel                : Double1D  (1..3);
        --satName            : String    (1..8);
        --epochDtg           : String    (1..17);
        --coordSys           : String    (1..5);
        --line1              : String    (1..512);
        --line2              : String    (1..512);

    -- Constructs 1P/2P cards from SPVEC data stored in the input arrays.
    -- This function only parses data from the input data but DOES NOT load/add the SPVEC to memory.
    -- Returned line1 and line2 will be empty if the function fails to construct the lines as requested.
    procedure SpVecArrayToLines
        (xa_spVec          : in      Double1D;
        xs_spVec           : in      String;
        line1              : out     String;
        line2              : out     String)
    with Import => True,
        Convention => C,
        External_Name => "SpVecArrayToLines";
        --xa_spVec           : Double1D  (1..512);
        --xs_spVec           : String    (1..512);
        --line1              : String    (1..512);
        --line2              : String    (1..512);

    -- Returns the first satKey from the currently loaded set of SpVec's that contains the specified satellite number
    -- This function is useful when SpVec DLL is used in applications that requires only one record (one SpVec entry) for one satellite and the applications refer to that SpVec by its satellite number. However, the Standardized Astrodynamic Algorithms library is only working with satKeys, this function helps to return the associated satKey of that satellite.
    function SpVecGetSatKey
        (satNum            :         Integer) return Long_Long_Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetSatKey";

    -- This function is similar to SpVecGetSatKey but designed to be used in Matlab. 
    -- This function is useful when SpVec DLL is used in applications that requires only one record (one SpVec entry) for one satellite and the applications refer to that SpVec by its satellite number. However, the Standardized Astrodynamic Algorithms library is only working with satKeys, this function helps to return the associated satKey of that satellite.
    procedure SpVecGetSatKeyML
        (satNum            :         Integer;
        satKey             : out     Long_Long_Integer)
    with Import => True,
        Convention => C,
        External_Name => "SpVecGetSatKeyML";

    -- Computes a satKey from the input data
    -- This is the proper way to reconstruct a satKey from its fields. If the users use their own routine to do this, the computed satKey might be different.
    function SpVecFieldsToSatKey
        (satNum            :         Integer;
        epochDtg           : in      String) return Long_Long_Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecFieldsToSatKey";
        --epochDtg           : String    (1..20);

    -- This function is similar to SpVecFieldsToSatKey but designed to be used in Matlab. 
    -- This is the proper way to reconstruct a satKey from its fields. If the users use their own routine to do this, the computed satKey might be different.
    procedure SpVecFieldsToSatKeyML
        (satNum            :         Integer;
        epochDtg           : in      String;
        satKey             : out     Long_Long_Integer)
    with Import => True,
        Convention => C,
        External_Name => "SpVecFieldsToSatKeyML";
        --epochDtg           : String    (1..20);

    -- Adds an SpVec using its individually provided field values
    function SpVecAddSatFrArray
        (xa_spVec          : in      Double1D;
        xs_spVec           : in      String) return Long_Long_Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecAddSatFrArray";
        --xa_spVec           : Double1D  (1..512);
        --xs_spVec           : String    (1..512);

    -- Adds an SpVec using its individually provided field values
    procedure SpVecAddSatFrArrayML
        (xa_spVec          : in      Double1D;
        xs_spVec           : in      String;
        satKey             : out     Long_Long_Integer)
    with Import => True,
        Convention => C,
        External_Name => "SpVecAddSatFrArrayML";
        --xa_spVec           : Double1D  (1..512);
        --xs_spVec           : String    (1..512);

    -- Updates existing SPVEC data with the provided new data stored in the input parameters. Note: satNum, epoch string can't be updated.
    function SpVecUpdateSatFrArray
        (satKey            :         Long_Long_Integer;
        xa_spVec           : in      Double1D;
        xs_spVec           : in      String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecUpdateSatFrArray";
        --xa_spVec           : Double1D  (1..512);
        --xs_spVec           : String    (1..512);

    -- Retrieves SPVEC data and stored it in the passing parameters
    function SpVecDataToArray
        (satKey            :         Long_Long_Integer;
        xa_spVec           : out     Double1D;
        xs_spVec           : out     String) return Integer
    with Import => True,
        Convention => C,
        External_Name => "SpVecDataToArray";
        --xa_spVec           : Double1D  (1..512);
        --xs_spVec           : String    (1..512);


end SpVecDll;

-- ========================= End of auto generated code ==========================
