...
{
   public static partial class SaasWrapper
   {
      // Provide the path to the dll
      const string SaasDll = "saas";
      ...
      /// <returns>0 if Saas.dll is initialized successfully, non-0 if there is an error</returns>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern int SaasInit(long apAddr);
      ...
      /// </summary>
      /// <param name="infoStr">A string to hold information about the Saas.dll (out-Character[128])</param>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern void SaasGetInfo(byte[] infoStr);
      ...
      /// <param name="xa_saasrun">run parameters (in-Double[8])</param>
      /// <param name="xa_msl">missile profile data.  see XA_MSL_? (in-Double[32])</param>
      /// <param name="xa_ls">launch site data.  see XA_LS_? (in-Double[8])</param>
      /// <param name="xa_satPen">array that stores data of satellite penetrations.  see XA_SATPEN_? (out-Double[*, 16])</param>
      /// <param name="xa_saasRet">array that stores other SAAS output.  see XA_SAASRET_? (out-Double[16])</param>
      /// <returns>0 if successfully, non-0 if there is an error</returns>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern int SaasGetResults(long satKey, double[] xa_saasrun, double[] xa_msl, double[] xa_ls, double[,] xa_satPen, double[] xa_saasRet);


      /// <summary>
      /// Computes auto cone size based on satellite height and missile profile      
      /// </summary>
      /// <param name="satHeight">satelite height (km) (in-Double)</param>
      /// <param name="xa_msl">missile profile data.  see XA_MSL_? (in-Double[32])</param>
      /// <returns>computed auto cone size (deg)</returns>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern double SaasAutoConeSize(double satHeight, double[] xa_msl);


      /// <summary>
      /// Computes missile time of flight based on satellite height and missile profile  
      /// </summary>
      /// <param name="satHeight">satellite height (km) (in-Double)</param>
      /// <param name="xa_msl">missile profile data.  see XA_MSL_? (in-Double[32])</param>
      /// <returns>computed missile flight time (sec)</returns>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern double SaasMissileFlightTime(double satHeight, double[] xa_msl);


      /// <summary>
      /// Computes launch orbital plane (inclination and node) based on the launch data
      /// </summary>
      /// <param name="xa_lnch">The input launch data (see XA_LNCH_? for array arrangement) (in-Double[8])</param>
      /// <param name="incli">The output planer's inclination (deg) (out-Double)</param>
      /// <param name="node">The output planer's node (deg) (out-Double)</param>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern void ComputeLnchOrbPlane(double[] xa_lnch, out double incli, out double node);
      ...
      /// </summary>
      /// <param name="satKey">The satellite's unique key (in-Long)</param>
      /// <param name="xa_plnr">The input new launch and time of flight data.  see XA_PLNR_? (in-Double[16])</param>
      /// <param name="xa_intxn">The output planer intersection data.  see XA_INTXN_? (out-Double[16])</param>
      /// <returns>0 if successfully, non-0 if there is an error</returns>
      [DllImport(SaasDll, CallingConvention = CallingConvention.Cdecl)]
      public static extern int FindPlanerIntersection(long satKey, double[] xa_plnr, double[] xa_intxn);
