...
// </table>
void BatchDCGetParams(int xai_ctrl[256], double xar_ctrl[256], char xas_ctrl[512]);

// Sets BatchDC parameter data (P Card) - all fields
// See BatchDCGetParams for the structure of the xai_ctrl and xar_ctrl arrays.
void BatchDCSetParams(int xai_ctrl[256], double xar_ctrl[256], char xas_ctrl[512]);

// Returns DC control parameters using array format
void BatchDCGetCtrlArr(double xa_dcCtrl[64]);

// Sets DC control parameters using array format
int BatchDCSetCtrlArr(double xa_dcCtrl[64]);
...
int BatchDCInitSat(long satKey, int* numObs, long obsKeys[], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);

// Initializes DC parameters for the satellite using only the provided/preselected obsKeys (not all loaded obs) (Thread-Safe)
// See BatchDCGetParams for the structure of the xai_dcElts and xar_dcElts arrays.
int BatchDCInitSatObsKeys(long satKey, double xa_dcCtrl[64], int numObs, long obsKeys[], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);
...
int BatchDCInitSatObsSel(long satKey, double xa_dcCtrl[64], double xa_selob[128], int* numObs, long obsKeys[], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);
...
// </table>
int BatchDCSolve(long satKey, int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);
...
int BatchDCSolveSelObs(long satKey, long obsKeys[], int numObs, int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);

// Removes a satellite, represented by the satKey, from BatchDC's memory
int BatchDCRemoveSat(long satKey);
...
// <br>
// See BatchDCSolve for the structure of the xar_dcElts array.
int BatchDCIterate(long satKey, double xa_ObsRes[][100], int xa_rejFlg[][32], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);

// Returns a concatenated string representation of a VCM generated by a successful DC
int BatchDCGetVcm(long satKey, char vcmLines[4000]);
...
int BatchDCGetAccptCrit(long satKey, double xa_ac[64]);
...
int SpToEGP(long satKey, double xa_egpCtrl[64], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);

// Performs batch-least-square differential corrections to the specified satellite (VCM/SPVEC) and
// returns the corrected elements SGP4/SGP4-XP in form of a TLE
// Note: No need to initialize VCM before calling this method
int SpToTle(long satKey, double xa_egpCtrl[64], char line1[512], char line2[512]);

// Combines SpToEGP and SpToTle functions into just one function call
// Note: No need to initialize VCM before calling this method
int SpToTleComb(long satKey, double xa_egpCtrl[64], char line1[512], char line2[512], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);

// Performs batch-least-square differential corrections to the specified satellite (VCM/SPVEC) and
// returns the corrected elements SGP4/SGP4-XP in form of a TLE
// Note: No need to initialize VCM before calling this method
int SpToCsv(long satKey, double xa_egpCtrl[64], char csvLine[512]);

// Combines SpToEGP and SpToCsv functions into just one function call
// Note: No need to initialize VCM before calling this method
int SpToCsvComb(long satKey, double xa_egpCtrl[64], char csvLine[512], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);
...
// returns the corrected SGP4/SGP4-XP elements and related data
int ExtEphemToEGP(char extEphFile[512], double xa_egpCtrl[64], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);
...
int IomodDC(int numObs, long obsKeys[], double xa_iomdc[64], int xai_dcElts[256], double xar_dcElts[256], char xas_dcElts[512]);
// Different DC propagation method
// DC propagator method is GP
#define DCPROPTYPE_GP   0
// DC propagator method is SGP4-XP
#define DCPROPTYPE_XP   4
// DC propagator method is SP
#define DCPROPTYPE_SP  99

// DC iterating returned code
// DC was successful
#define ITERCODE_DONE       0
// DC has error
#define ITERCODE_ERROR      1
// DC is still iterating
#define ITERCODE_ITERATING  2
// DC diverged but trying to recover
#define ITERCODE_DIVERGED   3

// indexes represent epoch placement options
// Nodal crossing nearest latest obs
#define EPFLG_NODALXINGLATESTOB  0
// Exact time of latest obs
#define EPFLG_LATESTOB           1
// Nodal crossing closest to specified time
#define EPFLG_NODALXINGATTIME    2
// Do not change epoch
#define EPFLG_ATEPOCH            3
// Exact at specified time
#define EPFLG_ATSPECIFIEDTIME    4
// Middle of obs span
#define EPFLG_MIDDLEOBSSPAN      5
// Exact time of earliest obs
#define EPFLG_EARLIESTOB         6
// Beginning of day of Epoch, good for eGP
#define EPFLG_BEGINDAYEPOCH      7
// Beginning of day of Last obs
#define EPFLG_BEGINDAYLATESTOB   8
// Nodal crossing nearest epoch, good for eGP
#define EPFLG_NODALXINGEPOCH     9
...
#define DC_PRINT_FIRSTBEST     0
// print every pass and summary
#define DC_PRINT_ALLPASS       1
// print summary only
#define DC_PRINT_SUMONLY       2
// print output elemenets only
#define DC_PRINT_ELTONLY       3
// do not print anything
#define DC_PRINT_NONE          4

// Iteration summary options
// print summary every iteration in Keplerian elts
#define ITER_SUM_KEP           1
// print summary every iteration in Equinoctial elts
#define ITER_SUM_EQNX          2

// indexes for integer data fields
// DC print option
#define XAI_CTRL_PRINTOPTION    0
// Apply biases from sensor file
#define XAI_CTRL_DEBIASOBS      1
// Weighed DC flag
#define XAI_CTRL_WEIGHTEDDC     2
// Epoch placement flag - see EPFLG_? for available options
#define XAI_CTRL_EPOCHOPTION    3
// Element correction flag - Ag
#define XAI_CTRL_CORRECT_AG     4
// Element correction flag - Af
#define XAI_CTRL_CORRECT_AF     5
// Element correction flag - Psi
#define XAI_CTRL_CORRECT_PSI    6
// Element correction flag - Chi
#define XAI_CTRL_CORRECT_CHI    7
// Element correction flag - L
#define XAI_CTRL_CORRECT_L      8
// Element correction flag - N
#define XAI_CTRL_CORRECT_N      9
// Element correction flag - B* (SGP4) B (SP)
#define XAI_CTRL_CORRECT_B     10
// Element correction flag - Agom (SP)
#define XAI_CTRL_CORRECT_AGOM  11
// Correction order
#define XAI_CTRL_CORRECTORDER  12
// Correct by the specified correction order for 1 iteration only
#define XAI_CTRL_FOR1ITERONLY  13
// Flag specifies which residuals are used for RMS calculation and convergence
#define XAI_CTRL_RESIDSELECT   14
// Flag; if set, produce sensor performance analysis summary
#define XAI_CTRL_SENPERFORM    15
// Flag; if set, deweight according to # of obs per track
#define XAI_CTRL_DWOBSPERTRCK  16
// Iteration summary control
#define XAI_CTRL_ITERSUMOPT    17
// Partials method
#define XAI_CTRL_PARTIALMETH   18
// Light time correction control
#define XAI_CTRL_LTC           19
// Number of iteration to allow no auto rejection of residuals
#define XAI_CTRL_BRUCE         20
// Propagation method - GP=0, XP=4, SP=99
#define XAI_CTRL_PROPMETHOD    21
// Flag; if set, correct B* vs X
#define XAI_CTRL_CORRECTBVSX   22
// Max # of iterations before declaring divergence
#define XAI_CTRL_MAXOFITERS    23
// Use predicted RMS versus previous RMS for convergence testing
#define XAI_CTRL_USEPREDRMS    24
// Residual computation method (GP only): DELTA/427M=1, SPADOC4=2
#define XAI_CTRL_RESCOMPMETH   25
// Flag; if set, use angle rates (obstypes = 4, 11)
#define XAI_CTRL_USEANGRATES   26

#define XAI_CTRL_SIZE          256



// indexes for real data fields
// RMS multiplier
#define XAR_CTRL_RMSMULT           0
// Time of specified epoch
#define XAR_CTRL_USEREPOCH         1
// Convergence criteria on time correction (%)
#define XAR_CTRL_CNVCRITONT        2
// First pass delta-t rejection criteria
#define XAR_CTRL_1STPASDELTAT      3
// Convergence criteria on 7-elt correction (%)
#define XAR_CTRL_CNVCRITON7ELT     4
// reset value for B term in subset correction
#define XAR_CTRL_BRESET            5
#define XAR_CTRL_SIZE            256


// indexes for accessing DC's integer data
// satellite number
#define XAI_DCELTS_SATNUM          0
// elset number
#define XAI_DCELTS_ELSETNUM        1
// elset's orbital/element type - see ELTTYPE_ constants for a list of possible values
#define XAI_DCELTS_ORBTYPE         2
// propagation method - see PROPTYPE_ constants for a list of possible values (GP=1, SP=2, External Ephemeris=3)
#define XAI_DCELTS_PROPTYPE        3
// spectr mode
#define XAI_DCELTS_SPECTR          4
// epoch revolution number
#define XAI_DCELTS_REVNUM          5
// correction type: 0=TIME, 1=PLANE", 2=7-ELT, 3=IN-TRK, 4=8-ELT, 5=SUBELT; see CORT_? for available values
#define XAI_DCELTS_CORRTYPE        6

// total iteration count
#define XAI_DCELTS_ITERCOUNT      10
// sub iteration count
#define XAI_DCELTS_SUBITER        11
// # residual accepted
#define XAI_DCELTS_RESACC         12
// # residual rejected
#define XAI_DCELTS_RESREJ         13

// 20-28 correction element flags
#define XAI_DCELTS_CORRFLGS       20

#define XAI_DCELTS_SIZE          256

// indexes for accessing DC's real data
// elset's epoch time in days since 1950 UTC
#define XAR_DCELTS_EPOCHDS50UTC    0
// n-dot/2  (for SGP, eph-type = 0)
#define XAR_DCELTS_NDOT            1
// n-double-dot/6  (for SGP, eph-type = 0)
#define XAR_DCELTS_N2DOT           2
// either SGP4 bStar (1/er) or SGP4-XP/SP bTerm (m2/kg)
#define XAR_DCELTS_BFIELD          3
// SGP4-XP/SP agom (m**2/kg)
#define XAR_DCELTS_AGOM            4
// SP outgassing parameter (km/s**2)
#define XAR_DCELTS_OGPARM          5
// semi major axis (km)
#define XAR_DCELTS_KEP_A           6
// eccentricity
#define XAR_DCELTS_KEP_E           7
// inclination (deg)
#define XAR_DCELTS_KEP_INCLI       8
// mean anamoly (deg)
#define XAR_DCELTS_KEP_MA          9
// right ascension of the ascending node (deg)
#define XAR_DCELTS_KEP_NODE       10
// argument of perigee (deg)
#define XAR_DCELTS_KEP_OMEGA      11
// AF
#define XAR_DCELTS_EQNX_AF        12
// AG
#define XAR_DCELTS_EQNX_AG        13
// CHI
#define XAR_DCELTS_EQNX_CHI       14
// PSI
#define XAR_DCELTS_EQNX_PSI       15
// mean longitude (deg)
#define XAR_DCELTS_EQNX_L         16
// mean motion (revs/day)
#define XAR_DCELTS_EQNX_N         17
// ECI posX (km)
#define XAR_DCELTS_POSX           18
// ECI posY (km)
#define XAR_DCELTS_POSY           19
// ECI posZ (km)
#define XAR_DCELTS_POSZ           20
// ECI velX (km/s)
#define XAR_DCELTS_VELX           21
// ECI velY (km/s)
#define XAR_DCELTS_VELY           22
// ECI velZ (km/s)
#define XAR_DCELTS_VELZ           23

// RMS (km)
#define XAR_DCELTS_RMS            40
// unweighted RMS (km)
#define XAR_DCELTS_RMSUNWTD       41
// delta T RMS (min)
#define XAR_DCELTS_DELTATRMS      42
// beta RMS (deg)
#define XAR_DCELTS_BETARMS        43
// delta height RMS (km)
#define XAR_DCELTS_DELTAHTRMS     44
// convergence value (km)
#define XAR_DCELTS_CONVQLTY       45
// predicted RMS (km)
#define XAR_DCELTS_RMSPD          46

// covariance diagonal L
#define XAR_DCELTS_COVL           60
// covariance diagonal N
#define XAR_DCELTS_COVN           61
// covariance diagonal CHI
#define XAR_DCELTS_COVCHI         62
// covariance diagonal PSI
#define XAR_DCELTS_COVPSI         63
// covariance diagonal AF
#define XAR_DCELTS_COVAF          64
// covariance diagonal AG
#define XAR_DCELTS_COVAG          65
// covariance diagonal BTERM
#define XAR_DCELTS_COVBTERM       66
// covariance not used
#define XAR_DCELTS_COVNA          67
// covariance diagonal AGOM
#define XAR_DCELTS_COVAGOM        68

// max partial residual (km)
#define XAR_DCELTS_XMAX           70
// max velocity resid (km/sec)
#define XAR_DCELTS_XMAX2          71
// max beta residual (deg)
#define XAR_DCELTS_XMAX3          72
// max delta-t residual (min)
#define XAR_DCELTS_XMAX4          73
// low argument of latitude coverage (deg)
#define XAR_DCELTS_PATCL          74
// high argument of latitude coverage (deg)
#define XAR_DCELTS_PATCH          75

// equinoctial covariance matrix - the lower triangular half 200-244
#define XAR_DCELTS_EQNXCOVMTX    200

#define XAR_DCELTS_SIZE          256

// indexes for accessing obs rejection flags
// satellite has decayed at the time of obs
#define XA_REJFLG_DECAYED         0
// obs residual computation error code
#define XA_REJFLG_ERROR           1
// right ascension residual rejected
#define XA_REJFLG_RA              2
// beta residual rejected
#define XA_REJFLG_BETA            3
// declination residual rejected
#define XA_REJFLG_DEC             4
// delta h residual rejected
#define XA_REJFLG_HEIGHT          5
// range residual rejected
#define XA_REJFLG_RANGE           6
// range rate residual rejected
#define XA_REJFLG_RR              7
// delta t residual rejected
#define XA_REJFLG_TIME            8

#define XA_REJFLG_SIZE           32

// indexes for accessing DC's acceptance criteria data
// standard - days from epoch
#define XA_AC_STD_EPOCH       0
// standard - number of residuals
#define XA_AC_STD_NORES       1
// standard - percent residual
#define XA_AC_STD_PRCNTRES    2
// standard - RMS (km)
#define XA_AC_STD_RMS         3
// standard - obs span (days)
#define XA_AC_STD_OBSSPAN     4
// standard - change in plan (deg)
#define XA_AC_STD_DELTAW      5
// standard - change in abar
#define XA_AC_STD_DELTAABAR   6
// standard - change in N (rev/day)
#define XA_AC_STD_DELTAN      7
// standard - change in B term
#define XA_AC_STD_DELTAB      8

// actual - days from epoch
#define XA_AC_ACT_EPOCH      20
// actual - number of residuals
#define XA_AC_ACT_NORES      21
// actual - percent residual
#define XA_AC_ACT_PRCNTRES   22
// actual - RMS (km)
#define XA_AC_ACT_RMS        23
// actual - obs span (days)
#define XA_AC_ACT_OBSSPAN    24
// actual - change in plan (deg)
#define XA_AC_ACT_DELTAW     25
// actual - change in abar
#define XA_AC_ACT_DELTAABAR  26
// actual - change in N (rev/day)
#define XA_AC_ACT_DELTAN     27
// actual - change in B term
#define XA_AC_ACT_DELTAB     28

#define XA_AC_SIZE           64


// indexes for accessing DC's string character data - index to index of a substring
// satellite's international designator
#define XAS_DCELTS_SATNAME_0TO7    0

#define XAS_DCELTS_SIZE          512

// Different DC control modes
// Use DC global settings settings
#define DCCTRL_MODE_GLOBAL    0
// Use provided/local DC settings
#define DCCTRL_MODE_LOCAL     1
...
#define XA_DCCTRL_MODE               0
// propagation method: SGP4 = 0, SP = 99, XP = 4, or see DCPROPTYPE_? for available options
#define XA_DCCTRL_PROPMETHOD 		  1
// debias obs flag: 0= do not debias obs, 1= debias obs
#define XA_DCCTRL_DEBIASOBS    		  2
// element correction order
#define XA_DCCTRL_CORRECTORDER		  3
// epoch placement flag - see EPFLG_? for available options
#define XA_DCCTRL_EPOCHOPTION  		  4
// use predicted RMS versus previous RMS for convergenece testing
#define XA_DCCTRL_USEPREDRMS		     5
// residual computation selection
#define XA_DCCTRL_RESIDSELECT        6
// correct by the specified correction order for 1 iteration only
#define XA_DCCTRL_FOR1ITERONLY		  7
// max # of iterations before declaring divergence
#define XA_DCCTRL_MAXOFITERS         8
// apply weighed DC flag
#define XA_DCCTRL_WEIGHTEDDC		     9
// light time correction control
#define XA_DCCTRL_LTC               10
// number of iteration to allow no auto rejection of residuals
#define XA_DCCTRL_BRUCE             11
// flag; if set, deweight according to # of obs per track
#define XA_DCCTRL_DWOBSPERTRCK      12
// partials method
#define XA_DCCTRL_PARTIALMETH       13
// Element correction flag - Ag
#define XA_DCCTRL_CORRECT_AG    	 20
// Element correction flag - Af
#define XA_DCCTRL_CORRECT_AF    	 21
// Element correction flag - Psi
#define XA_DCCTRL_CORRECT_PSI   	 22
// Element correction flag - Chi
#define XA_DCCTRL_CORRECT_CHI   	 23
// Element correction flag - L
#define XA_DCCTRL_CORRECT_L     	 24
// Element correction flag - N
#define XA_DCCTRL_CORRECT_N     	 25
// Element correction flag - B* (SGP4) B (SP)
#define XA_DCCTRL_CORRECT_B     	 26
// Element correction flag - Agom (SP)
#define XA_DCCTRL_CORRECT_AGOM  	 27
// Convergence criteria on time (%) [5.0%]
#define XA_DCCTRL_CNVCRITONT        30
// First pass delta-t rejection criteria Convergence criteria on time correction (min) [20 minutes]
#define XA_DCCTRL_1STPASDELTAT      31
// Convergence criteria on 7-elt correction (%) [1.0%]
#define XA_DCCTRL_CNVCRITON7ELT     32
// RMS multiplier [4.0]
#define XA_DCCTRL_RMSMULT           33
// reset value for B term in subset correction [0.01]
#define XA_DCCTRL_BRESET            34
// user's specified epoch - only when epoch placement option = 4 exact at specified time
#define XA_DCCTRL_USEREPOCH         35
// SP only - density consider parameter
#define XA_DCCTRL_CONSIDER          40
// GP only - residual computation method
#define XA_DCCTRL_GPRCM 			    40
// GP only - SGP4: correct B* vs X, SGP4-XP: correct B
#define XA_DCCTRL_CORRECTBVSX		 41

#define XA_DCCTRL_SIZE              64

// Indexes of paramters using in IomodDC()
// Iomod/DC control mode (not yet used)
#define XA_IOMDC_MODE             0
// DC element type, see DCPROPTYPE_? for available options
#define XA_IOMDC_DCELTTYPE        1
// Epoch placement flag - see EPFLG_? for available options
#define XA_IOMDC_EPFLG            2
// Time of specified Epoch in Ds50UTC (only for XA_EGPCTRL_EPFLG = 2 or 4)
#define XA_IOMDC_NEWEPOCH         3
// Correction order
#define XA_IOMDC_ORDERCOR         4
...
#define XA_IOMDC_OBSSELMODE       5
// residual computation selection
#define XA_IOMDC_RESIDSELECT      6
// correct by the specified correction order for 1 iteration only
#define XA_IOMDC_FOR1ITERONLY     7
// max # of iterations before declaring divergence
#define XA_IOMDC_MAXOFITERS       8
// apply weighed DC flag
#define XA_IOMDC_WEIGHTEDDC		  9
// light time correction control
#define XA_IOMDC_LTC             10
// number of iteration to allow no auto rejection of residuals
#define XA_IOMDC_BRUCE           11
// flag; if set, deweight according to # of obs per track
#define XA_IOMDC_DWOBSPERTRCK    12
// partials method
#define XA_IOMDC_PARTIALMETH     13
// debias obs flag: 0= do not debias obs, 1= debias obs
#define XA_IOMDC_DEBIASOBS    	 14
// use predicted RMS versus previous RMS for convergenece testing
#define XA_IOMDC_USEPREDRMS		 15

// Element correction flag - Ag
#define XA_IOMDC_CORRECT_AG    	 20
// Element correction flag - Af
#define XA_IOMDC_CORRECT_AF    	 21
// Element correction flag - Psi
#define XA_IOMDC_CORRECT_PSI   	 22
// Element correction flag - Chi
#define XA_IOMDC_CORRECT_CHI   	 23
// Element correction flag - L
#define XA_IOMDC_CORRECT_L     	 24
// Element correction flag - N
#define XA_IOMDC_CORRECT_N     	 25
// Element correction flag - B* (SGP4) B (SP)
#define XA_IOMDC_CORRECT_B     	 26
// Element correction flag - Agom (SP)
#define XA_IOMDC_CORRECT_AGOM  	 27
// Convergence criteria on time (%) [5.0%]
#define XA_IOMDC_CNVCRITONT      30
// First pass delta-t rejection criteria Convergence criteria on time correction (min) [20 minutes]
#define XA_IOMDC_1STPASDELTAT    31
// Convergence criteria on 7-elt correction (%) [1.0%]
#define XA_IOMDC_CNVCRITON7ELT   32
// RMS multiplier [4.0]
#define XA_IOMDC_RMSMULT         33
// reset value for B term in subset correction [0.01]
#define XA_IOMDC_BRESET          34
// SP only - density consider parameter
#define XA_IOMDC_CONSIDER        40
// GP only - residual computation method
#define XA_IOMDC_GPRCM 			 40
// GP only - SGP4: correct B* vs X, SGP4-XP: correct B
#define XA_IOMDC_CORRECTBVSX		 41

// see IOMDC_METHOD_? for available options
#define XA_IOMDC_METHOD          50
