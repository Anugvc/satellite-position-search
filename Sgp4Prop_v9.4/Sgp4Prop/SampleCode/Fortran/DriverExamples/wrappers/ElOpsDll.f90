...
function ElOpsInit(apAddr) bind(C, name = "ElOpsInit") result(errCode)
   integer :: errCode	! 0 if the ElOps dll is initialized successfully, non-0 if there is an error
   integer(8), value :: apAddr   ! The handle that was returned from DllMainInit()
end function ElOpsInit
!*******************************************************************************



! Returns information about the current version of ElOps.dll. The information is placed in the string parameter you pass in
subroutine ElOpsGetInfo(infoStr) bind(C, name = "ElOpsGetInfo")
   character, intent(out) :: infoStr(128)   ! A string to hold the information about ElOps.dll
end subroutine ElOpsGetInfo
!*******************************************************************************



! Checks to see if satellite has geo orbit
function IsGeoOrbit(incli, period) bind(C, name = "IsGeoOrbit") result(isGeo)
   integer :: isGeo	! Return 1 if satellite has geo orbit, 0 if it doesn't
   real(8), value :: incli   ! satellite's inclination (deg)
   real(8), value :: period   ! satellite's period (min)
end function IsGeoOrbit
!*******************************************************************************



! Estimates the approx long east subpt
function CompLonEastSubPt(ds50UTC, node, omega, mnAnomaly) bind(C, name = "CompLonEastSubPt") result(lonE)
   real(8) :: lonE	! estimated long east sub point (deg)
   real(8), value :: ds50UTC   ! time in days since 1950, UTC
   real(8), value :: node   ! right ascension of the ascending node (deg)
   real(8), value :: omega   ! argument of perigee (deg)
   real(8), value :: mnAnomaly   ! mean anomaly (deg)
end function CompLonEastSubPt
!*******************************************************************************



! Computes the decay time of the input satellite
function FindSatDecayTime(satKey, f10Avg, decayDs50UTC) bind(C, name = "FindSatDecayTime") result(errCode)
   integer :: errCode	! Returns 0 if success, 1 if nDot/2 lt 0, 2 if mean motion lt 1.5, and 3 if f2log lt 0
   integer(8), value :: satKey   ! The input satKey of the satellite needs to compute decay time
   real(8), value :: f10Avg   ! Input F10 average value
   real(8), intent(out) :: decayDs50UTC   ! The output decay time in days since 1950 UTC
end function FindSatDecayTime
!*******************************************************************************



! Returs parameters of a satellite via its satKey
function GetSatParameters(satKey, xa_satparm) bind(C, name = "GetSatParameters") result(errCode)
   integer :: errCode	! 0 if the satellite parameters are successfully retrieved, non-0 if there is an error
   integer(8), value :: satKey   ! The input satKey of the satellite needs to compute gobs parameters
   real(8), intent(out) :: xa_satparm(32)   ! Output satellite's parameters.  see XA_SATPARM_?
end function GetSatParameters
!*******************************************************************************



! Returs the satellite number associated with the input satKey
function SatNumOf(satKey) bind(C, name = "SatNumOf") result(satNum)
   integer :: satNum	! The satellite number associated with the input satKey (satNum = -1 if satKey doesn't exist)
   integer(8), value :: satKey   ! The input satKey
end function SatNumOf
!*******************************************************************************
...
function AddManeuverVPStr(satKey, vpString) bind(C, name = "AddManeuverVPStr") result(errCode)
   integer :: errCode	! 0 if the impulsive maneuver is added successfully, non-0 if there is an error.
   integer(8), value :: satKey   ! The unique key of the satellite of which impulsive maneuver is added to
   character, intent(in) :: vpString(512)   ! Impulse manuever using VP-card string format
end function AddManeuverVPStr
!*******************************************************************************
...
! Note: All maneuvers have to be entered before the satellite's initialization step
function AddManeuverVPArr(satKey, xa_vp) bind(C, name = "AddManeuverVPArr") result(errCode)
   integer :: errCode	! 0 if the impulsive maneuver is added successfully, non-0 if there is an error.
   integer(8), value :: satKey   ! The unique key of the satellite of which impulsive maneuver is added to
   real(8), intent(in) :: xa_vp(16)   ! Impulsive maneuver using VP-card array format - see XA_VP_? for array arrangement
end function AddManeuverVPArr
!*******************************************************************************
