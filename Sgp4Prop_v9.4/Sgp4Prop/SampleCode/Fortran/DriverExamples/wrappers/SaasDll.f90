...
function SaasInit(apAddr) bind(C, name = "SaasInit") result(errCode)
   integer :: errCode	! 0 if Saas.dll is initialized successfully, non-0 if there is an error
   integer(8), value :: apAddr   ! The handle that was returned from DllMainInit()
end function SaasInit
!*******************************************************************************
...
subroutine SaasGetInfo(infoStr) bind(C, name = "SaasGetInfo")
   character, intent(out) :: infoStr(128)   ! A string to hold information about the Saas.dll
end subroutine SaasGetInfo
!*******************************************************************************
...
!    
function SaasGetResults(satKey, xa_saasrun, xa_msl, xa_ls, xa_satPen, xa_saasRet) bind(C, name = "SaasGetResults") result(errCode)
   integer :: errCode	! 0 if successfully, non-0 if there is an error
   integer(8), value :: satKey   ! The satellite's unique key
   real(8), intent(in) :: xa_saasrun(8)   ! run parameters
   real(8), intent(in) :: xa_msl(32)   ! missile profile data.  see XA_MSL_?
   real(8), intent(in) :: xa_ls(8)   ! launch site data.  see XA_LS_?
   real(8), intent(out) :: xa_satPen(16, *)   ! array that stores data of satellite penetrations.  see XA_SATPEN_?
   real(8), intent(out) :: xa_saasRet(16)   ! array that stores other SAAS output.  see XA_SAASRET_?
end function SaasGetResults
!*******************************************************************************



! Computes auto cone size based on satellite height and missile profile      
function SaasAutoConeSize(satHeight, xa_msl) bind(C, name = "SaasAutoConeSize") result(coneSize)
   real(8) :: coneSize	! computed auto cone size (deg)
   real(8), value :: satHeight   ! satelite height (km)
   real(8), intent(in) :: xa_msl(32)   ! missile profile data.  see XA_MSL_?
end function SaasAutoConeSize
!*******************************************************************************



! Computes missile time of flight based on satellite height and missile profile  
function SaasMissileFlightTime(satHeight, xa_msl) bind(C, name = "SaasMissileFlightTime") result(flightTime)
   real(8) :: flightTime	! computed missile flight time (sec)
   real(8), value :: satHeight   ! satellite height (km)
   real(8), intent(in) :: xa_msl(32)   ! missile profile data.  see XA_MSL_?
end function SaasMissileFlightTime
!*******************************************************************************



! Computes launch orbital plane (inclination and node) based on the launch data
subroutine ComputeLnchOrbPlane(xa_lnch, incli, node) bind(C, name = "ComputeLnchOrbPlane")
   real(8), intent(in) :: xa_lnch(8)   ! The input launch data (see XA_LNCH_? for array arrangement)
   real(8), intent(out) :: incli   ! The output planer's inclination (deg)
   real(8), intent(out) :: node   ! The output planer's node (deg)
end subroutine ComputeLnchOrbPlane
!*******************************************************************************



! Compares a satellite elset against a new launch to find planer intersection time and associated data
function FindPlanerIntersection(satKey, xa_plnr, xa_intxn) bind(C, name = "FindPlanerIntersection") result(errCode)
   integer :: errCode	! 0 if successfully, non-0 if there is an error
   integer(8), value :: satKey   ! The satellite's unique key
   real(8), intent(in) :: xa_plnr(16)   ! The input new launch and time of flight data.  see XA_PLNR_?
   real(8), intent(out) :: xa_intxn(16)   ! The output planer intersection data.  see XA_INTXN_?
end function FindPlanerIntersection
!*******************************************************************************
