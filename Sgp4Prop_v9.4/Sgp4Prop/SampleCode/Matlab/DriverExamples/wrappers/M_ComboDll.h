...
// numOfEntryExits    Number of times when the primary and secondary satellites enter and exit the "Maximum separation for relative minima" (out-Integer)
// returns 0 if the COMBO data was computed successfully, non-0 if there is an error
int ComboCompPriSec_MT(double* xa_cb_parms, __int64 priSatKey, __int64 secSatKey, double* absMinDs50UTC, double* absMinDist, double* relMinDs50UTCs, int* numOfRelMins, double* entryDs50UTCs, double* exitDs50UTCs, int* numOfEntryExits);
...
int ComboPOC(double* priSatPos, double* priSatVel, double* priSatCov, double priSatRad, double* secSatPos, double* secSatVel, double* secSatCov, double secSatRad, int xf_CovType, double* xa_cb_poc);
...
int ComboCSMPOC(char* csmFile, double sumR, double* xa_cb_poc);
