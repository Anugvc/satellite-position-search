...
int Sgp4LoadFileAll(char* sgp4InputFile);
...
int Sgp4SaveFile(char* sgp4File, int saveMode, int saveForm);
...
// vel                Resulting ECI velocity vector (km/s) in True Equator and Mean Equinox of Epoch. (out-Double[3])
// returns 0 if the propagation is successful, non-0 if there is an error (see error decoder in GP_ERR_?).
int Sgp4PropDs50UtcPosVel(__int64 satKey, double ds50UTC, double* pos, double* vel);
...
int Sgp4PosVelToKep(int yr, double day, double* pos, double* vel, double* posNew, double* velNew, double* xa_kep);
...
// returns 0 if the reepoch is successful, non-0 if there is an error.
int Sgp4ReepochCsv(__int64 satKey, double reEpochDs50UTC, char* csvLine);
...
// returns 0 if the propagation is successful, non-0 if there is an error (see error decoder in GP_ERR_?).
int Sgp4GenEphems(__int64 satKey, double startTime, double endTime, double stepSize, int sgp4_ephem, int arrSize, double* ephemArr, int* genEphemPts);
...
// genEphemPts        Actual number of ephemeris points generated (always &le; arrSize) (out-Integer)
// returns 0 if the propagation is successful, non-0 if there is an error (see error decoder in GP_ERR_?).
int Sgp4GenEphems_OS(double* xa_tle, double startTime, double endTime, double stepSize, int sgp4_ephem, int arrSize, double* ephemArr, int* genEphemPts);
...
int Sgp4PropAllSats(__int64* satKeys, int numOfSats, double ds50UTC, double* ephemArr);
...
// returns 0 if the reepoch is successful, non-0 if there is an error.
int XpGetNativeElts(__int64 satKey, double ds50UTC, double* xa_eqnx, double* xa_eqnx_dot);
...
// xa_eqnx_dot        Equinoctial Element rates (subtract earth rotation rate from Longitude Rate) at reEpoch time (out-Double[6])
// returns 0 if the propagation is successful, non-0 if there is an error.
int XpReepochGetNativeElts(__int64 satKey, double reEpochDs50UTC, char* csvLine, double* xa_eqnx, double* xa_eqnx_dot);
