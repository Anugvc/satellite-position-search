...
int LamodSenSatVisible(__int64 senKey, __int64 satKey);
...
__int64 LamodInitSenSat(__int64 senKey, __int64 satKey);
...
// satKey             The satellite's unique key (in-Long)
// returns Returns a senSatKey: gt 0: Success, = 0: Warning, lt 0: Error
__int64 LamodInitSenSat_MT(double* xa_la_parms, __int64 senKey, __int64 satKey);
...
int LamodGetSenSatDataAll(__int64 senSatKey, int* stepMode, int* optVisFlg, double* startDs50TAI, double* stopDs50TAI, double* interval, double* period);
...
int LamodSenSatDataToArray(__int64 senSatKey, double* xa_senSat);
...
int LamodComputeLookAngle(__int64 senSatKey, double currDs50TAI, int* lookCode, double* xa_look, double* senElts, double* satElts);
...
int LamodGetOrbSenViewdata(__int64 senSatKey, double* uvSun, double* uvMoon, double* xa_angle, int* nBore1, int* nBore2, double* xa_offBore);
...
// xa_lv              The array that stores resulting look and view data, see XA_LV_? for array arrangement (out-Double[128])
void LamodSenSatDirect_OS(double ds50TAI, double* xa_locSen, double* xa_pvSat, double* xa_lv);
