...
// vel                the resulting velocity vector (km/s) (out-Double[3])
// returns 0 if the computation is successful, non-0 if there is an error
int Iomod_OS(double* xa_senLoc, int numObs, double* xa_obsArr, double* timeDs50UTC, double* pos, double* vel);
...
void AngleBetweenObs(__int64 obsKey1, __int64 obsKey2, double* angSep, int* errCode);
